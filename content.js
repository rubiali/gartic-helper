const words = [
    "007",
    "13 fantasmas",
    "2 cachorros bobos",
    "2 filhos de francisco",
    "2001 uma odisseia no espaco",
    "300",
    "7 caixas",
    "a arca de noe",
    "a bailarina",
    "a barraca do beijo",
    "a batalha de riddick",
    "a bela e a fera",
    "a bolha assassina",
    "a bruxa de blair",
    "a bruxinha sabrina",
    "a bussola de ouro",
    "a cadeira do diabo",
    "a caixa",
    "a casa amaldicoada",
    "a casa caiu",
    "a casa da colina",
    "a casa das coelhinhas",
    "a casa de cera",
    "a casa de vidro",
    "a casa do lago",
    "a casa do mickey mouse",
    "a casa dos espiritos",
    "a casa dos passaros mortos",
    "a casa monstro",
    "a chave magica",
    "a chave mestra",
    "a chegada",
    "a coisa",
    "a colheita do mal",
    "a conquista da honra",
    "a conquista do paraiso",
    "a cor de um crime",
    "a cor purpura",
    "a corrente do bem",
    "a corrente do mal",
    "a creche do papai",
    "a culpa e das estrelas",
    "a dama de ferro",
    "a dama dourada",
    "a dama oculta",
    "a enfermeira betty",
    "a era do gelo",
    "a escola de susto do gasparzinho",
    "a esfera",
    "a espera de um milagre",
    "a espia",
    "a estrada",
    "a estrada da vida",
    "a experiencia",
    "a familia addams",
    "a familia camundongo",
    "a familia da noiva",
    "a familia do futuro",
    "a fantastica fabrica de chocolate",
    "a feiticeira",
    "a filha do presidente",
    "a forma da agua",
    "a freira",
    "a fuga das galinhas",
    "a garota da capa vermelha",
    "a garota dinamarquesa",
    "a hora de voltar",
    "a hora do pesadelo",
    "a hora do recreio",
    "a hora do rush",
    "a ilha",
    "a invasao",
    "a invasao das aranhas gigantes",
    "a lagoa azul",
    "a lenda do cavaleiro sem cabeca",
    "a lenda do tesouro perdido",
    "a lenda do zorro",
    "a lenda dos guardioes",
    "a liga extraordinaria",
    "a lista de schindler",
    "a luta pela esperanca",
    "a mao assassina",
    "a mao que balanca o berco",
    "a mascara do zorro",
    "a menina e o cavalo",
    "a mentira",
    "a morte lhe cai bem",
    "a morte pede carona",
    "a morte te da parabens",
    "a mulher de preto",
    "a mulher invisivel",
    "a mumia",
    "a nevoa",
    "a noite do jogo",
    "a noiva cadaver",
    "a noiva de chucky",
    "a nova cinderela",
    "a nova escola do imperador",
    "a nova onda do imperador",
    "a novica rebelde",
    "a origem",
    "a outra",
    "a paixao de cristo",
    "a pantera cor-de-rosa",
    "a pequena sereia",
    "a praia",
    "a princesa prometida",
    "a procura da felicidade",
    "a profecia",
    "a proposta",
    "a rede social",
    "a rocha",
    "a setima alma",
    "a sogra",
    "a sombra do guerreiro",
    "a sombra e a escuridao",
    "a teoria de tudo",
    "a troca",
    "a turma do bairro",
    "a ultima musica",
    "a ultima sessao de cinema",
    "a vaca e o frango",
    "a viagem de chihiro",
    "a vida moderna de rocko",
    "a vida nao e um sonho",
    "a vida secreta das abelhas",
    "a vila",
    "a volta do todo poderoso",
    "a volta dos mortos vivos",
    "aang",
    "abacate",
    "abacaxi",
    "abaco",
    "abada",
    "abafado",
    "abafador",
    "abafar",
    "abaixar",
    "abajur",
    "abalroar",
    "abanar",
    "abandonado",
    "abandonar",
    "abastecer",
    "abatido",
    "abduzir",
    "abelha",
    "abencoado",
    "abencoar",
    "aberto",
    "abetarda",
    "abismado",
    "abismo do medo",
    "abiu",
    "abobadado",
    "abobora",
    "abobora jacarezinho",
    "abobrinha",
    "aborrecer",
    "aborrecido",
    "abotoadura",
    "abotoar",
    "abracadabra",
    "abracar",
    "abrico",
    "abridor de garrafa",
    "abridor de latas",
    "abrigar",
    "abril despedacado",
    "abril sangrento",
    "abrir",
    "absorver",
    "abutre",
    "academia",
    "acafrao",
    "acai",
    "acalmar",
    "acampamento",
    "acampamento do papai",
    "acampar",
    "acanhado",
    "acaraje",
    "acaro",
    "ace ventura",
    "acebolado",
    "aceitar",
    "acelerar",
    "acelga",
    "acenar",
    "acender",
    "acerola",
    "acertar",
    "acervo",
    "acessar",
    "achar",
    "achatado",
    "achatar",
    "achocolatado",
    "acidentado",
    "acidente",
    "acido",
    "acinturado",
    "acinzentado",
    "aclive",
    "acne",
    "acolchoado",
    "acolher",
    "acomodar",
    "aconchegar",
    "aconselhar",
    "acor",
    "acordar",
    "acostamento",
    "acougue",
    "acougueiro",
    "acrilico",
    "acrobata",
    "acucar",
    "acucar mascavo",
    "acucareiro",
    "acude",
    "acusado",
    "acusar",
    "adaga",
    "adao leao",
    "adaptador",
    "adax",
    "adega",
    "adesivo",
    "adestrador",
    "adestrar",
    "adeus, lenin!",
    "adiantado",
    "adiar",
    "adicionar",
    "admirado",
    "admirar",
    "adocante",
    "adocar",
    "adolepeixes",
    "adolescente",
    "adorador",
    "adormecer",
    "adubar",
    "adubo",
    "adulto",
    "advogado",
    "advogado do diabo",
    "aerofolio",
    "aeromoca",
    "aeroplano",
    "aeroporto",
    "afagar",
    "afastado",
    "afastar",
    "afiado",
    "afiador de facas",
    "afiar",
    "afluente",
    "afogado",
    "afta",
    "afundado",
    "afundar",
    "agachado",
    "agachar",
    "agarrado",
    "agasalhado",
    "agasalhar",
    "agendar",
    "agitado",
    "aglomerar",
    "agogo",
    "agora voce ve",
    "agressivo",
    "agriao",
    "agricultor",
    "agronomo",
    "agrotoxico",
    "agrupar",
    "agua",
    "agua com gas",
    "agua de coco",
    "agua para elefantes",
    "agua-viva",
    "aguar",
    "aguardente",
    "aguia",
    "agulha",
    "agulha de croche",
    "agulhao vela",
    "agulheiro",
    "agumon",
    "aie-aie",
    "aipim",
    "aipo",
    "aizen sousuke",
    "ajoelhado",
    "ajoelhar",
    "ajudante",
    "ajudante de papai noel",
    "ajudar",
    "ajustar",
    "akamaru",
    "aladdin",
    "aladin",
    "alado",
    "alagar",
    "alambique",
    "alambrado",
    "alameda",
    "alargador",
    "alargar",
    "alarme",
    "alaude",
    "alavanca",
    "albatroz",
    "albergue",
    "album",
    "alca",
    "alcachofra",
    "alcancar",
    "alcaparra",
    "alcateia",
    "alce",
    "alcool",
    "aldrava",
    "alecrim",
    "alegrar",
    "alem da vida",
    "alergia",
    "alface",
    "alfaiate",
    "alfajor",
    "alfinete",
    "algema",
    "algodao",
    "algodao doce",
    "alguem tem que ceder",
    "alho",
    "alho-poro",
    "alianca",
    "alicate",
    "alicate de unha",
    "alice no pais das maravilhas",
    "alien vs predador",
    "alimentar",
    "alinhado",
    "alinhar",
    "alisar",
    "aljava",
    "almas condenadas",
    "almeirao",
    "almocar",
    "almoco",
    "almofada",
    "almondega",
    "almoxarife",
    "alone in the dark",
    "alongamento",
    "alongar",
    "alpaca",
    "alpinista",
    "alpiste",
    "alto",
    "alto falante",
    "altura",
    "alugar",
    "alvin e os esquilos",
    "amaciar",
    "amador",
    "amadurecer",
    "amanhecer",
    "amante",
    "amar",
    "amaranto",
    "amarelinha",
    "amargo",
    "amarrado",
    "amarrar",
    "amarrotado",
    "amarrotar",
    "amassado",
    "amassar",
    "ambientalista",
    "ambrosia",
    "ambulancia",
    "ameiva",
    "ameixa",
    "amendoa",
    "amendoim",
    "american pie",
    "amido de milho",
    "amigas para sempre",
    "amigo",
    "amigo secreto",
    "amizade",
    "amizade colorida",
    "amolado",
    "amor alem da vida",
    "amor sem escalas",
    "amora",
    "amoroso",
    "amortecedor",
    "ampulheta",
    "amuleto",
    "amy",
    "ana pimentinha",
    "anaconda",
    "anao",
    "ancora",
    "ancorar",
    "andador",
    "andaime",
    "andar",
    "andorinha",
    "androide 18",
    "andy panda",
    "anel",
    "aneleira",
    "anemona",
    "angel beats",
    "angelical",
    "angu",
    "anilha",
    "animado",
    "animais fantasticos e onde habitam",
    "animaniacs",
    "aniversariante",
    "anjo",
    "anjo maldito",
    "anjos da lei",
    "anjos da noite",
    "anjos da vida",
    "anjos do sol",
    "anjos e demonios",
    "annabelle",
    "anoitecer",
    "anotar",
    "anpanman",
    "antena",
    "antena parabolica",
    "antes do amanhecer",
    "antes que termine o dia",
    "antidoto",
    "antigo",
    "antilope",
    "antolho",
    "anu",
    "anzol",
    "ao entardecer",
    "apagador",
    "apaixonado",
    "apaixonar",
    "aparador",
    "aparecer",
    "aparelho ortodontico",
    "apartamento",
    "apenas um show",
    "apendicite",
    "apertado",
    "apertar",
    "apetitoso",
    "apicultor",
    "apimentado",
    "apimentar",
    "apitar",
    "apito",
    "aplaudir",
    "apocalypto",
    "apodrecer",
    "apollo 13",
    "apontador",
    "apontador de lapis",
    "apontar",
    "apos a morte",
    "aposentado",
    "apresentador",
    "apresentar",
    "apressado",
    "aprovar",
    "aproximar",
    "aqua teen",
    "aquaman",
    "aquamarine",
    "aquario",
    "aquarista",
    "aquatico",
    "aquecedor",
    "aquecer",
    "ar condicionado",
    "araca",
    "aracnofobia",
    "arame farpado",
    "aranha",
    "aranha-camelo",
    "aranha-caranguejeira",
    "aranha-do-mar",
    "aranha-marrom",
    "aranha-pavao",
    "araponga",
    "arapuca",
    "arar",
    "arara",
    "arara-vermelha",
    "ararajuba",
    "aratu",
    "arbitrar",
    "arbitro",
    "arborizar",
    "arbusto",
    "arca",
    "arca de noe",
    "arco",
    "arco e flecha",
    "arco-iris",
    "arder",
    "area de servico",
    "areia",
    "arenque",
    "ariel",
    "ariranha",
    "arlequina",
    "armadilha",
    "armadura",
    "armageddon",
    "armario",
    "armazem",
    "armeiro",
    "arminho",
    "aro",
    "arpao",
    "arqueiro",
    "arqueiro verde",
    "arqueologo",
    "arquibancada",
    "arquipelago",
    "arquiteto",
    "arquivista",
    "arquivo-x",
    "arraia",
    "arrancar",
    "arranha-ceu",
    "arranhar",
    "arrastar",
    "arraste-me para o inferno",
    "arremessar",
    "arremesso de peso",
    "arrepiado",
    "arrepiar",
    "arroz",
    "arroz doce",
    "arroz integral",
    "arroz tropeiro",
    "arruinado",
    "arrumadeira",
    "arteria",
    "artesao",
    "artista plastico",
    "arvore",
    "arvore de natal",
    "as aventuras de babar",
    "as aventuras de jackie chan",
    "as aventuras de pi",
    "as aventuras de tintim",
    "as branquelas",
    "as bruxas de salem",
    "as cronicas de narnia",
    "as cronicas de spiderwick",
    "as invasoes barbaras",
    "as loucuras de dick e jane",
    "as meninas superpoderosas",
    "as mumias do farao",
    "as mumias vivas",
    "as panteras",
    "as patricinhas de beverly hills",
    "as super gatinhas",
    "as tartarugas ninjas",
    "as trapalhadas de flapjack",
    "as trigemeas",
    "asa",
    "asa de frango",
    "asa delta",
    "ascensorista",
    "asfaltado",
    "asfaltar",
    "asfalto",
    "asma",
    "asno",
    "aspargo",
    "aspero",
    "aspersorio",
    "aspirador de po",
    "assadeira",
    "assado",
    "assaltar",
    "assalto sobre trilhos",
    "assar",
    "assistente social",
    "assistir",
    "assoalho",
    "assombracao",
    "assombrado",
    "assombrar",
    "assoprar",
    "assustado",
    "assustar",
    "asteroide",
    "astro boy",
    "astrolabio",
    "astronauta",
    "astronave",
    "astronomo",
    "atabaque",
    "atacar",
    "atadura",
    "ate a morte",
    "ate que a sorte nos separe",
    "atelie",
    "atemoia",
    "atendente",
    "aterrar",
    "aterrissar",
    "aterro sanitario",
    "atirador",
    "atirar",
    "atividade paranormal",
    "atleta",
    "atmosfera",
    "atoba",
    "atolado",
    "atomo",
    "ator",
    "atos que desafiam a morte",
    "atras",
    "atrasado",
    "atrasar",
    "atravessar",
    "atropelar",
    "atum",
    "auditorio",
    "aureola",
    "auroque",
    "aurora boreal",
    "autografo",
    "automobilismo",
    "autorama",
    "auxiliar",
    "auxiliar administrativo",
    "avalanche",
    "avarento",
    "avatar",
    "ave-do-paraiso",
    "aveia",
    "avela",
    "avental",
    "aventuras com os kratts",
    "avestruz",
    "aviao",
    "avisar",
    "avo",
    "axila",
    "axolote",
    "azaleia",
    "azedar",
    "azedo",
    "azeite",
    "azeite de dende",
    "azeitona",
    "azeitona preta",
    "azucrinar",
    "azul e a cor mais quente",
    "azulao",
    "azulejista",
    "azulejo",
    "baba",
    "baba de moca",
    "babado",
    "babador",
    "babao",
    "babar",
    "babuino",
    "baby looney tunes",
    "bacalhau",
    "bacamarte e chumbinho",
    "bacia",
    "backyardigans",
    "bacon",
    "bacteria",
    "bacuri",
    "bad boys",
    "badejo",
    "badminton",
    "bafometro",
    "bagageiro",
    "bagagem",
    "bagre",
    "baguete",
    "baguncado",
    "baguncar",
    "baiacu",
    "bailarina",
    "baioneta",
    "baixista",
    "bajular",
    "bakugan",
    "bala",
    "bala de coco",
    "bala de goma",
    "balaclava",
    "balaio",
    "balanca",
    "balancar",
    "balao",
    "balaustre",
    "balcao",
    "balconista",
    "balde",
    "bale",
    "baleia",
    "baleia-azul",
    "baleiro",
    "balsa",
    "bambam",
    "bambi",
    "bambole",
    "bambu",
    "banana",
    "banana boat",
    "banana split",
    "banana-da-terra",
    "banana-maca",
    "banana-nanica",
    "banana-prata",
    "bananada",
    "bananaman",
    "bananeira",
    "bancario",
    "banco",
    "banda",
    "bandana",
    "bandeira",
    "bandeirinha",
    "bandeja",
    "bandidas",
    "bando",
    "bandolim",
    "bangalo",
    "banheira",
    "banheiro",
    "banheiro quimico",
    "banho",
    "banjo",
    "banqueta",
    "banquete",
    "baqueta",
    "bar",
    "baralho",
    "barata",
    "barba",
    "barbante",
    "barbapapa",
    "barbatana",
    "barbeador",
    "barbeiro",
    "barbie",
    "barbudo",
    "barco",
    "bardock",
    "barista",
    "barman",
    "barney",
    "barra de cereais",
    "barraca",
    "barracao",
    "barranco",
    "barras paralelas",
    "barreira",
    "barriga",
    "barrigudo",
    "barril",
    "barro",
    "barry e a banda das minhocas",
    "bart",
    "barulhento",
    "basculante",
    "basquetebol",
    "bastao",
    "bastardos inglorios",
    "bata",
    "batalha naval",
    "batalhao",
    "batata",
    "batata baroa",
    "batata frita",
    "batata palha",
    "batata-doce",
    "batatinha",
    "batedeira",
    "batente",
    "bater",
    "bater ou correr",
    "bateria",
    "baterista",
    "batfino e karate",
    "batgirl",
    "batida",
    "batina",
    "batismo",
    "batismo de sangue",
    "batizar",
    "batman",
    "batman o cavaleiro das trevas",
    "batom",
    "battle royale",
    "batucada",
    "batucar",
    "batuta",
    "bau",
    "baunilha",
    "bauru",
    "bazuca",
    "beavis and butt-head",
    "bebado",
    "bebe",
    "bebedouro",
    "beber",
    "bebes geniais",
    "beca",
    "bee movie",
    "beethoven",
    "beija-flor",
    "beijar",
    "beijinho",
    "beijo do dragao",
    "beirute",
    "beisebol",
    "beleza americana",
    "beliche",
    "beliscao",
    "beliscar",
    "beluga",
    "bem-te-vi",
    "bem-vindo a selva",
    "ben 10",
    "bender",
    "bengala",
    "bengala doce",
    "bercario",
    "berco",
    "berimbau",
    "berinjela",
    "bermuda",
    "bernardo e bianca",
    "berrante",
    "berrar",
    "bertalha",
    "besouro",
    "besouro-do-amor",
    "besouro-girafa",
    "besouro-rinoceronte",
    "beterraba",
    "betoneira",
    "betta",
    "betty atomica",
    "betty boop",
    "beyblade",
    "bezerro",
    "biblia",
    "biblioteca",
    "bibliotecario",
    "bibo pai e bobi filho",
    "bicama",
    "bicar",
    "biceps",
    "bicho de pelucia",
    "bicho de sete cabecas",
    "bicho-da-seda",
    "bicho-folha",
    "bicho-papao",
    "bicho-pau",
    "bicho-preguica",
    "bicicleta",
    "bicicleta ergometrica",
    "bico",
    "bicudo, o lobisomem",
    "bide",
    "bife",
    "bife a milanesa",
    "bife a parmegiana",
    "bifurcar",
    "big ben",
    "bigode",
    "bigorna",
    "bilboque",
    "bilhete",
    "bilheteiro",
    "bilheteria",
    "billy",
    "binoculo",
    "biologo",
    "biombo",
    "biomedico",
    "bioquimico",
    "bipede",
    "biqueira",
    "biquini",
    "bird box",
    "birdman",
    "biruta",
    "bisao",
    "biscoito",
    "biscoito da sorte",
    "bisonho",
    "bisteca",
    "bisturi",
    "blade o cacador de vampiros",
    "blade runner",
    "bleach",
    "bloco",
    "blogueiro",
    "bloo",
    "bloqueado",
    "bloquear",
    "blusa",
    "boa",
    "boate",
    "bob esponja",
    "bob esponja o filme",
    "bob o construtor",
    "bobo de camarao",
    "boca",
    "bocal",
    "bocejar",
    "bochecha",
    "bocio",
    "bode",
    "boi",
    "boi-bumba",
    "boia",
    "boiadeiro",
    "boiar",
    "boina",
    "boitata",
    "bojack horseman",
    "bola",
    "bola de boliche",
    "bola de cristal",
    "bola de golfe",
    "bola de gude",
    "bolacha",
    "boleador",
    "bolero",
    "bolha",
    "boliche",
    "bolinha",
    "bolinho de arroz",
    "bolinho de bacalhau",
    "bolinho de chuva",
    "bolo",
    "bolo de abacaxi",
    "bolo de aipim",
    "bolo de aniversario",
    "bolo de brigadeiro",
    "bolo de cafe",
    "bolo de caja",
    "bolo de caju",
    "bolo de cenoura",
    "bolo de chocolate",
    "bolo de coco",
    "bolo de fuba",
    "bolo de kiwi",
    "bolo de laranja",
    "bolo de leite",
    "bolo de limao",
    "bolo de maracuja",
    "bolo de morango",
    "bolo de sorvete",
    "bolo formigueiro",
    "bolo marmore",
    "bolsa",
    "bolsa de palha",
    "bolsa termica",
    "bolso",
    "bomba",
    "bomba de ar",
    "bombacha",
    "bombardear",
    "bombeiro",
    "bombom",
    "bonde",
    "bondinho",
    "bondoso",
    "bone",
    "boneca",
    "boneco de neve",
    "bonequinha de luxo",
    "bongo",
    "bonita",
    "bonkers",
    "bonsai",
    "borboleta",
    "borboletario",
    "borbulhar",
    "bordadeira",
    "bordado",
    "bordar",
    "borracha",
    "borracheiro",
    "borrifador",
    "borrifar",
    "bosko",
    "bosque",
    "bota",
    "botao",
    "bote",
    "bote salva-vidas",
    "botijao de gas",
    "botina",
    "boto",
    "bouli",
    "boxe",
    "boxeador",
    "bracelete",
    "braco",
    "branca de neve e o cacador",
    "brandy e sr bigodes",
    "brasinhas do espaco",
    "bratz",
    "bravo",
    "breadwinners",
    "brega",
    "brevidade",
    "brigadeiro",
    "brilhante",
    "brilhar",
    "brilho labial",
    "brincalhao",
    "brincar",
    "brinco",
    "brindar",
    "brinquedo",
    "brinquedo assassino",
    "brioche",
    "britadeira",
    "broa",
    "broca",
    "broche",
    "brocolis",
    "bromelia",
    "bronze",
    "bronzeado",
    "bronzeador",
    "bronzear",
    "brotar",
    "broto de feijao",
    "brutus",
    "bruxa",
    "bucha",
    "bucky",
    "bueiro",
    "bufalo",
    "buffy a caca-vampiros",
    "bugio-preto",
    "bule",
    "bumblebee",
    "bumerangue",
    "buque",
    "buraco",
    "burca",
    "bureta",
    "burritos",
    "buscar",
    "buzina",
    "buzinar",
    "buzz lightyear",
    "cabana",
    "cabeca",
    "cabecear",
    "cabeleireiro",
    "cabelo",
    "cabelo de anjo",
    "cabeludo",
    "cabide",
    "cabideiro",
    "cabo",
    "cabo de guerra",
    "cabo do medo",
    "cabra",
    "cabresto",
    "cabrito",
    "caca ao tesouro",
    "caca submarina",
    "caca-fantasmas",
    "cacador",
    "cacadores de mente",
    "cacamba",
    "cacao",
    "cacar",
    "cacarola",
    "cacatua",
    "cacau",
    "cachaca",
    "cachalote",
    "cacheado",
    "cachear",
    "cachecol",
    "cachepo",
    "cachimbo",
    "cacho",
    "cachoeira",
    "cachorro",
    "cachorro quente",
    "cachorro-vinagre",
    "cacique",
    "cacto",
    "cadarco",
    "cadaveres",
    "cade a minha entrega?",
    "cadeado",
    "cadeira",
    "cadeira de balanco",
    "cadeira de praia",
    "cadeira de rodas",
    "cadeira eletrica",
    "cadeirante",
    "cadela",
    "caderneta",
    "caderno",
    "caes assassinos",
    "caes de aluguel",
    "cafe",
    "cafe com leite",
    "cafe da manha",
    "cafeteira",
    "cafeteria",
    "cafona",
    "cafune",
    "caiaque",
    "caibra",
    "caido",
    "caillou",
    "caipirinha",
    "caipora",
    "cair",
    "caititu",
    "caixa",
    "caixa de correio",
    "caixa de ferramentas",
    "caixa de fosforo",
    "caixa de isopor",
    "caixa de papelao",
    "caixa de som",
    "caixa registradora",
    "caixao",
    "caixote",
    "caja",
    "cajado",
    "cajamanga",
    "caju",
    "cajuzinho",
    "calabresa",
    "calado",
    "calango",
    "calar",
    "calau",
    "calca",
    "calcada",
    "calcadeira",
    "calcanhar",
    "calcar",
    "calcinha",
    "caldeirao",
    "caldeireiro",
    "caldo de cana",
    "caldo de carne",
    "caldo de galinha",
    "caldo de mocoto",
    "caldo verde",
    "caldo-de-feijao",
    "caleidoscopio",
    "calibrador",
    "calice",
    "calmo",
    "calopsita",
    "calor",
    "calorento",
    "calota",
    "calvicie",
    "cama",
    "cama elastica",
    "camada",
    "camafeu",
    "camaleao",
    "camarao",
    "camarao empanado",
    "camareira",
    "cambuca",
    "camburao",
    "camelo",
    "caminhando nas nuvens",
    "caminhao",
    "caminhao de lixo",
    "caminhao pipa",
    "caminhar",
    "caminho",
    "caminhoneiro",
    "caminhonete",
    "camisa",
    "camisa de forca",
    "camisa regata",
    "camiseta",
    "camisola",
    "camp rock",
    "campainha",
    "campo minado",
    "camu-camu",
    "camuflado",
    "camuflar",
    "camundongo",
    "cana-de-acucar",
    "canaleta",
    "canape",
    "canario",
    "canario negro",
    "canastra",
    "canavial",
    "candeeiro",
    "candelabro",
    "caneca",
    "canela",
    "caneleira",
    "canelone",
    "caneta",
    "caneta hidrocor",
    "canga",
    "canguru",
    "canguru jack",
    "canhao",
    "canhoto",
    "canibal",
    "canil",
    "canivete",
    "canja de galinha",
    "canjica",
    "cano",
    "canoa",
    "canoagem",
    "cansado",
    "cantando na chuva",
    "cantar",
    "cantil",
    "canto",
    "cantoneira",
    "cantor",
    "canudo",
    "cao de briga",
    "capa",
    "capa de chuva",
    "capacete",
    "capacho",
    "capataz",
    "capela",
    "capelete",
    "capelo",
    "capim",
    "capim santo",
    "capitao america",
    "capitao caverna",
    "capitao falange",
    "capitao planeta",
    "capivara",
    "capoeira",
    "capotar",
    "cappuccino",
    "capricornio",
    "capuz",
    "caqui",
    "cara ou coroa",
    "caracal",
    "caracol",
    "carambola",
    "caramelo",
    "caramujo",
    "carandiru",
    "carangos e motocas",
    "caranguejo",
    "caranguejo-dos-coqueiros",
    "caranguejo-violinista",
    "caravela",
    "carcereiro",
    "cardeal",
    "cardiologista",
    "cardume",
    "careca",
    "careta",
    "carga",
    "carga explosiva",
    "caricatura",
    "carimbar",
    "carimbo",
    "carinhoso",
    "carmen sandiego",
    "carnage",
    "carnaval",
    "carne",
    "carne de sol",
    "carne moida",
    "carneiro",
    "carnivoro",
    "caro",
    "carpa",
    "carpeta",
    "carpete",
    "carpinteiro",
    "carrapato",
    "carrapicho",
    "carregador de celular",
    "carregar",
    "carreta",
    "carretel",
    "carretilha",
    "carrica",
    "carrilhao",
    "carrinho",
    "carrinho de bate-bate",
    "carrinho de bebe",
    "carrinho de feira",
    "carrinho de mao",
    "carrinho de rolima",
    "carro",
    "carro funebre",
    "carro-forte",
    "carroca",
    "carrocinha",
    "carros",
    "carrossel",
    "carruagem",
    "carruagens de fogo",
    "carta",
    "cartao",
    "cartao de credito",
    "cartao de memoria",
    "cartao telefonico",
    "cartas na mesa",
    "cartas para julieta",
    "carteira",
    "carteiro",
    "cartola",
    "cartolina",
    "cartucho de tinta",
    "cartunista",
    "caruru",
    "carvao",
    "carvoeiro",
    "casa",
    "casa de areia",
    "casablanca",
    "casaco",
    "casadinho",
    "casamento",
    "casamento grego",
    "casar",
    "casca",
    "cascalho",
    "cascao",
    "cascata",
    "cascavel",
    "caseiro",
    "caspa",
    "casquinha",
    "casquinha de siri",
    "cassetete",
    "cassino",
    "cassino royale",
    "castanha",
    "castanho",
    "castanholas",
    "castelo",
    "castical",
    "castor",
    "casuar",
    "casula",
    "cat noir",
    "cata-vento",
    "catapora",
    "catapulta",
    "catarata",
    "catatau",
    "catdog",
    "catedral",
    "catraca",
    "caule",
    "cavaleiro",
    "cavalete",
    "cavalgar",
    "cavalo",
    "cavalo de fogo",
    "cavalo de pau",
    "cavalo-marinho",
    "cavanhaque",
    "cavaquinho",
    "cavar",
    "caveira",
    "caverna",
    "caverna do dragao",
    "caverninha",
    "caviar",
    "caxixi",
    "caxumba",
    "cd",
    "cebola",
    "cebola empanada",
    "cebola roxa",
    "cebolinha",
    "cegonha",
    "cegonha-bico-de-sapato",
    "celada",
    "celebrar",
    "celeiro",
    "celula",
    "celular",
    "cemiterio",
    "cenoura",
    "centauro",
    "centopeia",
    "central do brasil",
    "centralizar",
    "centurions",
    "cerca",
    "cerca eletrica",
    "cercar",
    "cereal",
    "cerebro",
    "cereja",
    "cerejeira",
    "ceroula",
    "certidao de nascimento",
    "cerveja",
    "cervo",
    "cesta",
    "cetro",
    "ceu",
    "cha",
    "cha de boldo",
    "cha de camomila",
    "cha de hortela",
    "cha gelado",
    "cha mate",
    "cha preto",
    "cha verde",
    "chacal",
    "chacoalhar",
    "chafariz",
    "chale",
    "chaleira",
    "chamar",
    "chamariz",
    "chamine",
    "champanhe",
    "champignon",
    "chantilly",
    "chao",
    "chapeu",
    "chapeu de aniversario",
    "chapinha de cabelo",
    "chapisco",
    "charango",
    "charley e mimmo",
    "charlie brown",
    "charlie e lola",
    "charque",
    "charrete",
    "charuto",
    "chatear",
    "chato",
    "chave",
    "chave canhao",
    "chave de boca",
    "chave de fenda",
    "chave de grifo",
    "chave de roda",
    "chave estrela",
    "chave inglesa",
    "chaveiro",
    "chaves",
    "cheddar",
    "cheesecake",
    "cheetara",
    "chefe",
    "chefe de cozinha",
    "cheio",
    "cheirar",
    "cheiro verde",
    "cheiroso",
    "cheque",
    "chester",
    "chi chi",
    "chicago",
    "chiclete",
    "chico bento",
    "chicoria",
    "chicote",
    "chifre",
    "chilly willy",
    "chimarrao",
    "chimbal",
    "chimichurri",
    "chimpanze",
    "chinchila",
    "chinelo",
    "chip",
    "chiqueiro",
    "chocalho",
    "chocolate",
    "chocolate amargo",
    "chocolate branco",
    "chocolate granulado",
    "chocolate quente",
    "chocotone",
    "chofer",
    "chopp",
    "choque",
    "chorar",
    "chowder",
    "chuchu",
    "chucrute",
    "chule",
    "chumaco",
    "chupeta",
    "chupim",
    "churrascaria",
    "churrasco",
    "churrasqueira",
    "churros",
    "chutar",
    "chuteira",
    "chuva",
    "chuva acida",
    "chuva negra",
    "chuveiro",
    "cicatriz",
    "cicatrizar",
    "ciclismo",
    "ciclope",
    "ciclovia",
    "cidadao kane",
    "cidade",
    "cidade de deus",
    "cidade dos anjos",
    "cidade dos homens",
    "cidade dos sonhos",
    "cidades de papel",
    "cidra",
    "cientista",
    "cigana",
    "cigarra",
    "cilindro",
    "cilios",
    "cilios posticos",
    "cimento",
    "cinderela",
    "cinegrafista",
    "cinema",
    "cinema paradiso",
    "cinema, aspirinas e urubus",
    "cinquenta tons de cinza",
    "cinta-liga",
    "cinto",
    "cinto de seguranca",
    "cinzel",
    "cipo",
    "circo",
    "circulo de fogo",
    "cirurgiao",
    "cirurgiao plastico",
    "cisco",
    "cisne",
    "cisne negro",
    "cisterna",
    "ciumento",
    "civeta",
    "claquete",
    "clara",
    "clarabela",
    "clarencio, o otimista",
    "clarinete",
    "claro",
    "classe",
    "claustrofobico",
    "clava",
    "clavicula",
    "clicar",
    "click",
    "cliente",
    "clifford",
    "clinica",
    "clip",
    "closer",
    "clube",
    "clube da luta",
    "coador",
    "coala",
    "cobertor",
    "cobra",
    "cobra-cega",
    "cobra-cipo",
    "cobra-coral",
    "cobrir",
    "cocada",
    "cocada preta",
    "cocar",
    "cochichar",
    "cochilar",
    "coco",
    "coco ralado",
    "codigo de barras",
    "codigo de honra",
    "codorna",
    "coelho",
    "coelho angora",
    "coelho da pascoa",
    "coelho osvaldo",
    "coelho ricochete",
    "coentro",
    "cofre",
    "cogumelo",
    "coifa",
    "coiote",
    "cola",
    "colar",
    "colcha",
    "colchao",
    "colchao inflavel",
    "colchonete",
    "coldre",
    "colecionar",
    "coleira",
    "colera",
    "colete",
    "colete cervical",
    "colher",
    "colher de pau",
    "colibri",
    "colirio",
    "coliseu de roma",
    "colmeia",
    "colorau",
    "colorido",
    "colorir",
    "coluna",
    "comandante",
    "comandar",
    "comando para matar",
    "combinar",
    "comboio",
    "combustivel",
    "comediante",
    "comedouro",
    "comemorar",
    "comer",
    "comerciante",
    "cometa",
    "cominho",
    "comissaria de bordo",
    "como caes e gatos",
    "como se fosse a primeira vez",
    "como treinar seu dragao",
    "comoda",
    "companhia",
    "comparar",
    "compasso",
    "comprar",
    "comprimido",
    "computador",
    "comungar",
    "concha",
    "concordar",
    "concreto",
    "condicionador",
    "condominio",
    "condor",
    "conduite",
    "cone",
    "conectado",
    "conectar",
    "confeiteiro",
    "confete",
    "confidente",
    "confiscar",
    "conformado",
    "confundir",
    "confuso",
    "conga",
    "congelado",
    "congelar",
    "conhaque",
    "conjuntivite",
    "conquistar",
    "consertar",
    "consolar",
    "constantine",
    "constelacao",
    "construir",
    "conta-gotas",
    "contagiar",
    "contagioso",
    "contaminar",
    "contente",
    "continente",
    "contorcionista",
    "contra file",
    "contrabaixo",
    "contrair",
    "contramao",
    "contrario",
    "contratar",
    "controle remoto",
    "conversando com deus",
    "conversar",
    "convidar",
    "convite",
    "cookies",
    "copeiro",
    "copia",
    "copo",
    "copo-de-leite",
    "coqueiro",
    "coquetel",
    "coqueteleira",
    "coracao",
    "coracao de dragao",
    "coracao de frango",
    "coracao de galinha",
    "coracao de tinta",
    "coracao gelado",
    "coracao louco",
    "coracao satanico",
    "coracao valente",
    "coragem, o cao covarde",
    "corais",
    "corajoso",
    "coraline e o mundo secreto",
    "corcova",
    "corcovado",
    "corcunda",
    "corda",
    "cordeiro",
    "coreografo",
    "coringa",
    "cornea",
    "corneta",
    "coroa",
    "corpete",
    "corrego",
    "corrente",
    "correr",
    "corretivo",
    "corrida alucinante",
    "corrida da morte",
    "corrida de cavalos",
    "corrida maluca",
    "corrida mortal",
    "corrimao",
    "corrupto",
    "corsario",
    "cortador de grama",
    "cortador de unha",
    "cortar",
    "cortina",
    "coruja",
    "corvina",
    "corvo",
    "cosmo",
    "costas",
    "costela",
    "costeleta",
    "costelinha",
    "costurar",
    "costureira",
    "cotoveleira",
    "cotovelo",
    "cotovia",
    "coturno",
    "couve",
    "couve-de-bruxelas",
    "couve-flor",
    "coveiro",
    "cowboy bebop",
    "cowboys and aliens",
    "cowboys de moo mesa",
    "cowboys do espaco",
    "coxa",
    "coxa de frango",
    "coxinha",
    "coyote",
    "cozido",
    "cozinha",
    "cozinhar",
    "cozinheiro",
    "craca",
    "cracha",
    "cranio",
    "crash",
    "cratera",
    "cravo",
    "creme de abobora",
    "creme de avela",
    "creme de leite",
    "creme de milho",
    "creme dental",
    "crepe",
    "crepusculo",
    "crescer",
    "criado-mudo",
    "crianca",
    "crimes no paraiso",
    "crina",
    "crista",
    "cristal",
    "cristaleira",
    "cristo redentor",
    "criticar",
    "crocodilo",
    "croissant",
    "cromado",
    "croquete",
    "cru",
    "crucificar",
    "crucifixo",
    "cruella",
    "cruz",
    "cruzamento",
    "cruzeta",
    "cubo",
    "cubo magico",
    "cuco",
    "cueca",
    "cuia",
    "cuica",
    "cuidador de idosos",
    "cuidar",
    "cultivar",
    "cumprimentar",
    "cupcake",
    "cupido",
    "cupim",
    "cupuacu",
    "curar",
    "curau",
    "curimata",
    "curio",
    "curioso",
    "curry",
    "curtindo a vida adoidado",
    "curtir",
    "curto",
    "curupira",
    "curva",
    "curvador de cilios",
    "curvar",
    "curvas perigosas",
    "cuscuz",
    "cutelo",
    "cutia",
    "cuticula",
    "cutucar",
    "cyberchase",
    "cyborg 009",
    "dado",
    "daltonico",
    "damas",
    "damasco",
    "danca com lobos",
    "danca das cadeiras",
    "danca dos mortos",
    "dancando no escuro",
    "dancando para a vida",
    "dancar",
    "dancarino",
    "danceteria",
    "danny phantom",
    "daphne blake",
    "dardo",
    "darkwing duck",
    "darwin",
    "datilografar",
    "datilografo",
    "dave, o barbaro",
    "de olhos bem fechados",
    "de pernas pro ar",
    "de repente e amor",
    "de volta a lagoa azul",
    "de volta para o futuro",
    "death note",
    "debrucar",
    "decolar",
    "decorador",
    "dedal",
    "dedetizador",
    "dedo",
    "dee dee",
    "degrau",
    "degustar",
    "deidara",
    "deitado",
    "deitar",
    "deja vu",
    "delegado",
    "deletar",
    "delineador",
    "demitir",
    "demolidor",
    "demolir",
    "dengue",
    "dennis, o pimentinha",
    "dentadura",
    "dente",
    "dentista",
    "depenado",
    "depilador eletrico",
    "depiladora",
    "depositar",
    "dermatologista",
    "derramar",
    "derreter",
    "derrubar",
    "desabamento",
    "desafiando gigantes",
    "desafiar",
    "desanimado",
    "desaparafusar",
    "desaparecer",
    "desarrumado",
    "desarrumar",
    "desastrado",
    "desbotado",
    "descabelado",
    "descabelar",
    "descalco",
    "descamar",
    "descansar",
    "descarga",
    "descascar",
    "descendentes",
    "descer",
    "descobrir",
    "descolorir",
    "desconectar",
    "desconfiar",
    "descongelar",
    "descosturar",
    "desejo proibido",
    "desenhar",
    "desenhista",
    "desenho",
    "desentupidor",
    "desentupir",
    "deserto",
    "desesperar",
    "desfiar",
    "desfilar",
    "desfile",
    "desinfetante",
    "desligado",
    "desligar",
    "desmaiar",
    "desmascarar",
    "desmatar",
    "desmontar",
    "desodorante",
    "despedir",
    "despencar",
    "despertador",
    "despertar",
    "desprezar",
    "destelhar",
    "destinos ligados",
    "destro",
    "destruir",
    "detector de fumaca",
    "detector de metais",
    "detergente",
    "detetive",
    "detona ralph",
    "deu a louca na chapeuzinho",
    "deu a louca na cinderela",
    "deus e brasileiro",
    "devagar",
    "devoto",
    "dia",
    "dia dos namorados macabro",
    "diabetes",
    "diabo-da-tasmania",
    "diabo-espinhoso",
    "dialogar",
    "diamante",
    "diamante de sangue",
    "diario",
    "diario de um banana",
    "diario de uma paixao",
    "diarios de motocicleta",
    "diarista",
    "dick vigarista",
    "difundir",
    "digerir",
    "digimon",
    "digitador",
    "digitar",
    "dinamite",
    "dingo",
    "dinheiro",
    "dino",
    "dinosaucers",
    "dinossauro",
    "diplomata",
    "dipper pines",
    "direita",
    "diretor",
    "dirigir",
    "dirigivel",
    "dirty dance",
    "disco",
    "disco voador",
    "discordar",
    "discutir",
    "disneylandia",
    "disquete",
    "distancia",
    "distraido",
    "distroller",
    "diva",
    "divergente",
    "divertidamente",
    "divertido",
    "dividir",
    "divorciar",
    "dj",
    "django",
    "do comeco ao fim",
    "do que as mulheres gostam",
    "do que os homens gostam",
    "dobradica",
    "dobradinha",
    "doce",
    "doce de abacaxi",
    "doce de abobora",
    "doce de banana",
    "doce de caju",
    "doce de coco",
    "doce de figo",
    "doce de jaca",
    "doce de kiwi",
    "doce de leite",
    "doce de mamao",
    "doce de manga",
    "doce de morango",
    "doce de pessego",
    "doceiro",
    "doces encontros",
    "docil",
    "docinho",
    "dodo",
    "dodoria",
    "doente",
    "doer",
    "doido",
    "doki",
    "doleiro",
    "doloroso",
    "dom pixote",
    "domado",
    "domador",
    "domar",
    "domestica",
    "domino",
    "dona flor e seus dois maridos",
    "doninha",
    "donnie darko",
    "dora, a aventureira",
    "doraemon",
    "dormindo com o inimigo",
    "dormindo fora de casa",
    "dorminhoco",
    "dormir",
    "doug",
    "dourado",
    "doutora brinquedos",
    "doze e demais",
    "doze homens e outro segredo",
    "draculaura",
    "draga",
    "dragao",
    "dragao do mar",
    "dragao vermelho",
    "dragao-de-komodo",
    "dragon ball",
    "dragon quest fly",
    "dragonball evolution",
    "dromedario",
    "droopy",
    "du, dudu e edu",
    "ducha",
    "duck dodgers",
    "ducktales",
    "duelo xiaolin",
    "duende",
    "duende verde",
    "dumbo",
    "duro de matar",
    "dvd",
    "e fada",
    "e o vento levou",
    "e se fosse verdade",
    "e.t.",
    "ebola",
    "echarpe",
    "eclipse",
    "eclipse mortal",
    "economista",
    "economizar",
    "edificio",
    "edredom",
    "educado",
    "educar",
    "edward elric",
    "edward maos de tesoura",
    "eek the cat",
    "efeito borboleta",
    "efeito domino",
    "egua",
    "ela",
    "ela e o cara",
    "ela e os caras",
    "elastico",
    "ele nao esta tao afim de voce",
    "elefante",
    "elegante",
    "elektra",
    "elenco",
    "eletricista",
    "elevador",
    "elfo",
    "eliminar",
    "elmo",
    "ema",
    "embaixador",
    "embalador",
    "embalagem",
    "embaracado",
    "embarcar",
    "embarque",
    "embriagado de amor",
    "embrulhado",
    "embrulhar",
    "emoji",
    "empacotado",
    "empacotar",
    "empada",
    "empadao",
    "empilhadeira",
    "empilhar",
    "empoeirado",
    "empolgado",
    "empresario",
    "emprestar",
    "empurrar",
    "emu",
    "encadernador",
    "encanador",
    "encardido",
    "encardir",
    "enchente",
    "encontro explosivo",
    "encorpado",
    "encostado",
    "encostar",
    "endivia",
    "energetico",
    "enfeitado",
    "enfeitar",
    "enfeite",
    "enfermeira",
    "engasgar",
    "engatar",
    "engenheiro",
    "engenheiro civil",
    "engenheiro de alimentos",
    "engenheiro mecanico",
    "engessado",
    "engolir",
    "engracado",
    "engravatar",
    "engraxate",
    "engrenagem",
    "enguia",
    "enjaulado",
    "enjoado",
    "enlouquecer",
    "enrolado",
    "enrolados",
    "ensaboado",
    "ensaboar",
    "ensinar",
    "entalado",
    "entediado",
    "enterrado vivo",
    "entrando numa fria",
    "entrar",
    "entre abelhas",
    "entre o ceu e a terra",
    "entre os muros da prisao",
    "entregador",
    "entregador de pizza",
    "entrevista com o vampiro",
    "entrevistador",
    "entrevistar",
    "entristecer",
    "entusiasmar",
    "envelhecer",
    "envelope",
    "envergonhado",
    "enviar",
    "enxada",
    "enxaguar",
    "enxame",
    "enxugar",
    "enxuto",
    "epiglote",
    "equilibrar",
    "equilibrista",
    "era uma vez em... hollywood",
    "era uma vez no oeste",
    "eric cartman",
    "erupcao",
    "erva-doce",
    "ervilha",
    "erza",
    "esbravejar",
    "esbugalhado",
    "escada",
    "escada rolante",
    "escalada",
    "escalar",
    "escaldar",
    "escalimetro",
    "escama",
    "escandaloso",
    "escapamento",
    "escapar",
    "escaravelho",
    "escargot",
    "escarola",
    "escavadeira",
    "escavar",
    "esclarecer",
    "escola",
    "escola de rock",
    "escola pra cachorro",
    "esconde-esconde",
    "esconder",
    "esconderijo",
    "escopeta",
    "escorpiao",
    "escorpiao-vinagre",
    "escorpioes a bordo",
    "escorredor de macarrao",
    "escorredor de pratos",
    "escorregador",
    "escorregar",
    "escoteiro",
    "escova",
    "escova de dente",
    "escovar",
    "escritor",
    "escritorio",
    "escrivaninha",
    "escrivao",
    "escudo",
    "esculpir",
    "escultor",
    "escultura",
    "escumadeira",
    "escuro",
    "esfigmomanometro",
    "esfinge",
    "esfirra",
    "esfregao",
    "esfregar",
    "esfriar",
    "esgoto",
    "esgrima",
    "esmagado",
    "esmagar",
    "esmalte",
    "esofago",
    "espada",
    "espaguete",
    "espalhar",
    "espanador",
    "espantado",
    "espantalho",
    "espantar",
    "esparadrapo",
    "espartilho",
    "espatula",
    "espelhado",
    "espelho",
    "espetado",
    "espetinho",
    "espeto",
    "espiao",
    "espiar",
    "espinafre",
    "espingarda",
    "espinha",
    "espinho",
    "espiral",
    "espiritos",
    "espirrar",
    "esponja",
    "esponja-do-mar",
    "esponjoso",
    "esporte sangrento",
    "esportista",
    "espreguicar",
    "espremedor de alho",
    "espremedor de frutas",
    "espremer",
    "espuma",
    "esquadrilha",
    "esquadrilha abutre",
    "esquadro",
    "esqueceram de mim",
    "esqueletico",
    "esqueleto",
    "esquerda",
    "esqui",
    "esquilo",
    "esquilo sem grilo",
    "esquilo-voador",
    "esquimo",
    "esquina",
    "esquistossomose",
    "estabilizador",
    "estaca",
    "estacao",
    "estacionamento",
    "estacionar",
    "estadio",
    "estado",
    "estagiario",
    "estampa",
    "estandarte",
    "estante",
    "estatua",
    "estatua da liberdade",
    "estatueta do oscar",
    "esteira",
    "esteira ergometrica",
    "estender",
    "esterilizador",
    "esteticista",
    "estetoscopio",
    "esticar",
    "estilete",
    "estilingue",
    "estilista",
    "estojo",
    "estojo de maquiagem",
    "estomago",
    "estorninho",
    "estourar",
    "estrada",
    "estrela",
    "estrela cadente",
    "estrela-do-mar",
    "estribo",
    "estudar",
    "estudioso",
    "estufa",
    "estufar",
    "estupido",
    "etiqueta",
    "eu os declaro marido e larry",
    "eu robo",
    "eu sou a lenda",
    "eu sou o maximo",
    "eu, eu mesmo e irene",
    "eufrazino",
    "eustacio bagge",
    "evaporar",
    "ever after high",
    "evidencias de um crime",
    "evocando espiritos",
    "evoluir",
    "exagerado",
    "exaustor",
    "excluido",
    "executivo",
    "exercito",
    "experimentar",
    "explodir",
    "explosao",
    "exportar",
    "exposicao",
    "extintor de incendio",
    "extrair",
    "extraordinario",
    "extraterrestre",
    "extrovertido",
    "fa",
    "fabrica",
    "fabricar",
    "faca",
    "facao",
    "facil",
    "fada",
    "fagote",
    "fairy tail",
    "faisao",
    "faisca",
    "faisca e fumaca",
    "faixa",
    "faixa de pedestre",
    "falar",
    "falcao",
    "fale com ela",
    "falir",
    "falsificado",
    "familia",
    "familia dracula",
    "familia urso",
    "family guy",
    "faminto",
    "famoso",
    "fanboy e chum chum",
    "fantasia",
    "fantasiar",
    "fantasma",
    "fantasmas nunca dormem",
    "fantoche",
    "faqueiro",
    "faquir",
    "farao",
    "faraonico",
    "farda",
    "farejar",
    "farinha",
    "farinheiro",
    "farmaceutico",
    "farmacia",
    "farofa",
    "farol",
    "farol de milha",
    "farolete",
    "farto",
    "fatal",
    "fate stay night",
    "fatia",
    "fatiar",
    "fauna",
    "favela",
    "fax",
    "faxineiro",
    "fazenda",
    "fazendeiro",
    "febre",
    "febre amarela",
    "febril",
    "fechadura",
    "fechar",
    "fedorento",
    "feijao",
    "feijao azuki",
    "feijao branco",
    "feijao fradinho",
    "feijao manteiga",
    "feijao preto",
    "feijoada",
    "feio",
    "feirante",
    "felicia",
    "felino",
    "feliz",
    "femur",
    "feneco",
    "fenix",
    "ferias frustradas",
    "ferir",
    "fermentar",
    "fermento",
    "feroz",
    "ferradura",
    "ferramenta",
    "ferrao",
    "ferreiro",
    "ferro de passar roupa",
    "ferrolho",
    "ferrorama",
    "fervente",
    "ferver",
    "festa",
    "festa junina",
    "festejar",
    "fiacao",
    "fibula",
    "fichario",
    "figa",
    "figado",
    "figo",
    "figurante",
    "figurinha",
    "figurinista",
    "fila",
    "filadelfia",
    "file",
    "file mignon",
    "filha",
    "filha da luz",
    "filho",
    "filmadora",
    "filmar",
    "filtro de agua",
    "filtro de linha",
    "filtro de papel",
    "filtro dos sonhos",
    "fim dos tempos",
    "financiar",
    "finn",
    "fino",
    "fio",
    "fio dental",
    "fique rico ou morra tentando",
    "fiscal",
    "fisiculturismo",
    "fisioterapeuta",
    "fita",
    "fita adesiva",
    "fita cassete",
    "fita crepe",
    "fita isolante",
    "fivela",
    "flacido",
    "flamingo",
    "flamula",
    "flan",
    "flanela",
    "flapjack",
    "flash",
    "flash gordon",
    "flauta",
    "flautista",
    "flecha",
    "flertar",
    "fliperama",
    "floquinho",
    "flor",
    "flor de cerejeira",
    "floresta",
    "floresta do mal",
    "floricultor",
    "floricultura",
    "florido",
    "florista",
    "florzinha",
    "flutuar",
    "fly",
    "foca",
    "foca-elefante",
    "focinheira",
    "focinho",
    "fofa",
    "fofo",
    "fofocar",
    "fofoqueiro",
    "fogao",
    "fogao a lenha",
    "fogazza",
    "fogo",
    "fogos de artificio",
    "fogueira",
    "foguete",
    "foice",
    "folgado",
    "folha",
    "fome animal",
    "fomos herois",
    "fondue",
    "fones de ouvido",
    "fonte da vida",
    "forma",
    "forma de gelo",
    "formao",
    "formar",
    "formiga",
    "formiga atomica ",
    "formiga-leao",
    "formiga-verde",
    "formigueiro",
    "formiguinhaz",
    "fornalha",
    "forno",
    "forno eletrico",
    "forrest gump",
    "fortalecer",
    "forte",
    "fotografar",
    "fotografo",
    "fraco",
    "fragata",
    "fragil",
    "frajola",
    "frajuto",
    "fralda",
    "framboesa",
    "frango",
    "frango a parmegiana",
    "frango a passarinho",
    "frango empanado",
    "frango-xadrez",
    "frangolino",
    "frangueira",
    "franja",
    "franjinha",
    "franzino",
    "frasco",
    "fratura",
    "freakazoid",
    "frear",
    "fred flintstone",
    "free willy",
    "freeza",
    "freezer",
    "freira",
    "frentista",
    "fresco",
    "frigideira",
    "frigobar",
    "frio",
    "fritar",
    "frito",
    "fronha",
    "frota",
    "frouxo",
    "frozen",
    "fruta-do-conde",
    "fruta-pao",
    "fruteira",
    "fuba",
    "fugir",
    "fuinha",
    "fullmetal alchemist",
    "fumaca",
    "fundo",
    "funghi",
    "funil",
    "funileiro",
    "furacao",
    "furadeira",
    "furado",
    "furao",
    "furar",
    "furgao",
    "furia de titas",
    "furioso",
    "fusivel",
    "futebol",
    "futebol americano",
    "futebol de areia",
    "futebol de botao",
    "futsal",
    "futurama",
    "fuzil",
    "fuzzy confusao",
    "gaara",
    "gabinete",
    "gabiroba",
    "gafanhoto",
    "gaguinho",
    "gaiola",
    "gaita",
    "gaivota",
    "galaxia",
    "galaxy trio",
    "galheteiro",
    "galho",
    "galinha",
    "galinha pintadinha",
    "galo",
    "galocha",
    "galopar",
    "gamao",
    "gamba",
    "gambit",
    "ganache",
    "gancho",
    "gandula",
    "gangorra",
    "gangues de nova york",
    "ganhar",
    "ganso",
    "ganza",
    "garafa pet",
    "garagem",
    "garapa",
    "garca",
    "garcom",
    "garconete",
    "garfield",
    "garfield - o filme",
    "garfo",
    "gargalhada",
    "garganta",
    "garganta do diabo",
    "gargantilha",
    "gargarejar",
    "gargulas",
    "gari",
    "garimpeiro",
    "garnet",
    "garoa",
    "garota interrompida",
    "garotas rebeldes",
    "garoupa",
    "garra",
    "garrafa",
    "garrafa pet",
    "garrafa termica",
    "garu",
    "gary",
    "gasoduto",
    "gasolina",
    "gasparzinho ",
    "gastrite",
    "gatilho",
    "gatinhar",
    "gato",
    "gato corajoso",
    "gato felix",
    "gato-do-mato",
    "gaucho",
    "gaveta",
    "gavial",
    "gaviao",
    "gazela",
    "gazela-girafa",
    "gel fixador",
    "geladeira",
    "gelado",
    "gelatina",
    "gelatina de morango",
    "geleia",
    "geleira",
    "gelo",
    "gema",
    "gemeos",
    "geneticista",
    "gengibre",
    "gengiva",
    "genio",
    "genio indomavel",
    "gente grande",
    "geografo",
    "geologo",
    "george, o rei da floresta",
    "gergelim",
    "geriatra",
    "gesso",
    "ghost do outro lado da vida",
    "ghoulia yelps",
    "gibao",
    "gibi",
    "gigante",
    "ginasta",
    "ginastica artistica",
    "gineta",
    "girafa",
    "girassol",
    "girino",
    "giroflex",
    "giz",
    "giz de cera",
    "glace",
    "gladiador",
    "globo",
    "gloss",
    "glub-glub",
    "glutao",
    "gnomo",
    "gnu",
    "godzilla",
    "gogeta",
    "gohan",
    "goiaba",
    "goiabada",
    "goku",
    "gola",
    "goleiro",
    "golfe",
    "golfinho",
    "gondola",
    "gongo",
    "gorduroso",
    "gorgonzola",
    "gorila",
    "gormiti",
    "gorpo",
    "gorro",
    "gostar",
    "gosto de sangue",
    "gostoso",
    "gota",
    "goteira",
    "goten",
    "governador",
    "governanta",
    "gps",
    "grade",
    "grafico",
    "grafitagem",
    "grafite",
    "grafiteiro",
    "gralha-azul",
    "gramofone",
    "grampeado",
    "grampeador",
    "grampear",
    "grampo de cabelo",
    "granada",
    "grande",
    "granizo",
    "granola",
    "grao de bico",
    "gratinar",
    "gravar",
    "gravata",
    "gravida",
    "gravidade",
    "graviola",
    "gravity falls",
    "green book",
    "grelha",
    "grelhar",
    "gremlins",
    "grilo",
    "grinalda",
    "gripe",
    "grisalho",
    "gritar",
    "groselha",
    "grosso",
    "grudento",
    "grumixama",
    "guacamole",
    "guanaco",
    "guandu",
    "guara",
    "guarana",
    "guarda florestal",
    "guarda-chuva",
    "guarda-roupa",
    "guarda-sol",
    "guardanapo",
    "guardar",
    "guarita",
    "guaxinim",
    "guelras",
    "guepardo",
    "guerra ao terror",
    "guerra dos mundos",
    "guerra nas estrelas",
    "guerrear",
    "guia de turismo",
    "guiar",
    "guidao",
    "guilhotina",
    "guinchar",
    "guincho",
    "guindaste",
    "guirlanda",
    "guitarra",
    "guitarrista",
    "guloso",
    "gumball",
    "habitantes da escuridao",
    "hairspray",
    "halloween - a noite do terror",
    "haltere",
    "halterofilismo",
    "hamburguer",
    "hamster",
    "hamtaro",
    "hancock",
    "handebol",
    "hannibal",
    "happy tree friends",
    "harpa",
    "harry e o balde de dinossauros",
    "harry potter",
    "he-man",
    "helice",
    "helicoptero",
    "heliponto",
    "hellboy",
    "hellraiser",
    "hellsing",
    "hepatite",
    "heroi",
    "hey arnold",
    "hi hi puffy amiyumi",
    "hidrante",
    "hidratante",
    "hidratar",
    "hiena",
    "high school musical",
    "highlander",
    "hipismo",
    "hipnotizado",
    "hipnotizar",
    "hipopotamo",
    "hipotalamo",
    "historia sem fim",
    "historinhas de dragoes",
    "hitch conselheiro amoroso",
    "holofote",
    "homem de ferro",
    "homem passaro",
    "homem-aranha",
    "homem-formiga",
    "homens de honra",
    "homens em furia",
    "homens, mulheres e filhos",
    "homer",
    "hong kong fu",
    "hop rebelde sem pascoa",
    "hoquei",
    "hora de aventura",
    "hora de voltar",
    "horrivel",
    "horta",
    "hortela",
    "hortelino",
    "hospicio",
    "hospital",
    "hostia",
    "hotel",
    "hotel transilvania",
    "hulk",
    "hunter x hunter",
    "iaque",
    "iate",
    "ibis",
    "iceberg",
    "identidade",
    "idolatrar",
    "iglu",
    "igreja",
    "iguana",
    "ikki",
    "ilha",
    "ilha do medo",
    "ilha dos desafios",
    "iluminado",
    "ilustrador",
    "ima",
    "imaginar",
    "imitar",
    "imortal",
    "impacto profundo",
    "impala",
    "imperio do sol",
    "imperio dos lobos",
    "impressora",
    "imprimir",
    "inalar",
    "incendiar",
    "incendio",
    "incensario",
    "incenso",
    "incinerar",
    "inclinado",
    "inclinar",
    "incolor",
    "incontrolavel",
    "indagar",
    "independence day",
    "indiana jones",
    "indicar",
    "indio",
    "infestar",
    "inflamavel",
    "inga",
    "ingresso",
    "inhame",
    "inimigos publicos",
    "injecao",
    "injetar",
    "inseticida",
    "inspecionar",
    "inspetor bugiganga",
    "inspirar",
    "instinto selvagem",
    "instrumentista",
    "instrutor",
    "inteligencia artificial",
    "inteligente",
    "interestelar",
    "interfonar",
    "interfone",
    "internet",
    "interprete",
    "interruptor",
    "intestino",
    "inundar",
    "inuyasha",
    "invasor zim",
    "invejoso",
    "inverno",
    "inverter",
    "ioga",
    "iogurte",
    "ioio",
    "irara",
    "irma",
    "irmao",
    "irmao do jorel",
    "irmaos metralha",
    "ironico",
    "irritado",
    "isca",
    "isqueiro",
    "itachi",
    "jabuti",
    "jabuticaba",
    "jaca",
    "jacana",
    "jacare",
    "jacu",
    "jacutinga",
    "jade chan",
    "jaguar",
    "jaguatirica",
    "jake",
    "jake long",
    "jaken",
    "jaleco",
    "jamanta",
    "jambo",
    "jambo e ruivao",
    "jamelao",
    "janela",
    "janela da alma",
    "janela indiscreta",
    "janela secreta",
    "jangada",
    "jantar",
    "jaqueta",
    "jararaca",
    "jararacucu",
    "jardim",
    "jardineira",
    "jardineiro",
    "jarra",
    "jaspion",
    "jatai",
    "jato",
    "jatoba",
    "jaula",
    "javali",
    "jay jay, o jatinho",
    "jean grey",
    "jeans",
    "jenipapo",
    "jerboa",
    "jerimum",
    "jessica rabbit",
    "jiboia",
    "jilo",
    "jim no mundo da lua",
    "jimbo",
    "jimmy neutron",
    "jiraiya",
    "jiu-jitsu",
    "joaninha",
    "joao e maria cacadores de bruxas",
    "joao grandao",
    "joao-de-barro",
    "joelheira",
    "joelho",
    "jogador de futebol",
    "jogador de volei",
    "jogador numero 1",
    "jogar",
    "jogo da memoria",
    "jogo de amor em las vegas",
    "jogo do amor",
    "jogo entre ladroes",
    "jogos mortais",
    "jogos vorazes",
    "johnny bravo",
    "johnny e june",
    "johnny test",
    "jonny quest",
    "joquei",
    "jorgen von estranho",
    "jornada nas estrelas",
    "jornal",
    "jornaleiro",
    "jornalista",
    "josie e as gatinhas",
    "jovem demais para morrer",
    "juanito jones",
    "judo",
    "juggernaut",
    "juiz",
    "jujuba",
    "julgar",
    "jumanji",
    "jumento",
    "jumper",
    "juniper lee",
    "juno",
    "jupiter",
    "jurassic park",
    "jurassico",
    "justica a qualquer preco",
    "justica de amor",
    "kafta",
    "kagome",
    "kakashi",
    "karate kid",
    "katara",
    "ken",
    "ketchup",
    "kevin levin",
    "kiara",
    "kiba",
    "kick buttowski",
    "kid flash",
    "kid vs. kat",
    "kill bill",
    "killer bee",
    "kim possible",
    "king kong",
    "king o rei da selva",
    "kissyfur",
    "kiwi",
    "konan",
    "korra",
    "kovu",
    "krypto",
    "kudu",
    "kunai",
    "kung fu panda",
    "kuririn",
    "la de aco",
    "la la land",
    "labios",
    "labirinto",
    "laboratorio",
    "laco",
    "lacraia",
    "lacrar",
    "lacre",
    "lacrimejar",
    "lactarista",
    "ladeira",
    "ladrao de diamantes",
    "ladroes de bicicleta",
    "lagarta",
    "lagartixa",
    "lagarto",
    "lago",
    "lagoa",
    "lagoona blue",
    "lagosta",
    "lagrima",
    "lagrimas do sol",
    "lajota",
    "lama",
    "lambari",
    "lamber",
    "lambreta",
    "lamentar",
    "lamina",
    "lampada",
    "lamparina",
    "lampiao",
    "lan house",
    "lanca",
    "lancamento de dardo",
    "lancamento de disco",
    "lancamento de martelo",
    "lancar",
    "lancha",
    "lanchar",
    "lancheira",
    "lanchonete",
    "lantejoula",
    "lanterna",
    "lanterna chinesa",
    "lanterna verde",
    "lapela",
    "lapis",
    "lapis de cor",
    "lapis de olho",
    "lapis labial",
    "lapis lazuli",
    "lapiseira",
    "laque",
    "laranja",
    "laranja mecanica",
    "lareira",
    "laringe",
    "larva",
    "lasanha",
    "lata",
    "latir",
    "lava",
    "lavar",
    "lavatorio",
    "lavrador",
    "leao",
    "leao da montanha",
    "leao-marinho",
    "lebre",
    "legalmente loira",
    "leiloar",
    "leitao",
    "leite",
    "leite condensado",
    "leite de coco",
    "leite de soja",
    "leite em po",
    "leiteira",
    "lembrancas",
    "lembrar",
    "lembrete",
    "leme",
    "lemure",
    "lenco",
    "lencol",
    "lenda urbana",
    "lenha",
    "lenhador",
    "lente",
    "lente de contato",
    "lentilha",
    "lento",
    "leoa",
    "leoes e cordeiros",
    "leoncio",
    "leopardo",
    "leopardo-das-neves",
    "leptospirose",
    "leque",
    "ler",
    "lesma",
    "letra e musica",
    "levantar",
    "leve",
    "levitar",
    "lex luthor",
    "lhama",
    "libelula",
    "libra",
    "licenca para casar",
    "lichia",
    "licor",
    "liga da justica",
    "ligado",
    "ligar",
    "ligeiramente gravidos",
    "ligeirinho",
    "lilica",
    "lilo e stitch",
    "lima",
    "limao",
    "limite vertical",
    "limonada",
    "limousine",
    "limpar",
    "limpo",
    "lince",
    "linda",
    "lindinha",
    "lingua",
    "lingua de sogra",
    "linguado",
    "linguica",
    "linha",
    "linhaca",
    "lion-o",
    "liquidificador",
    "lira",
    "lirio",
    "lisa",
    "lisbela e o prisioneiro",
    "liso",
    "lista telefonica",
    "listrado",
    "livraria",
    "livro",
    "lixa de unha",
    "lixar",
    "lixeira",
    "lobisomem",
    "lobo",
    "lobo da tasmania",
    "lobo-guara",
    "lobo-marinho",
    "locadora",
    "localizar",
    "locomotiva",
    "locutor",
    "logan",
    "loira",
    "loja",
    "lola bunny",
    "lola e virginia",
    "lolita",
    "lombo",
    "lombriga",
    "lona",
    "longo",
    "lontra",
    "loucademia de policia",
    "louco",
    "louro",
    "lousa",
    "louva-a-deus",
    "lua",
    "lua nova",
    "lucky star",
    "lucy",
    "luffy",
    "lula",
    "lula lele",
    "lula molusco",
    "luluzinha",
    "luminaria",
    "luneta",
    "lupa",
    "lustrar",
    "lustre",
    "lutar",
    "luva",
    "luva de boxe",
    "luzes da cidade",
    "mabeco",
    "mabel pines",
    "maca",
    "maca do amor",
    "maca verde",
    "macacao",
    "macaco",
    "macaco loco",
    "macaco-aranha",
    "macaco-narigudo",
    "macaco-prego",
    "macadamia",
    "macaneta",
    "macarico",
    "macarrao",
    "macarrao instantaneo",
    "macarronada",
    "machado",
    "machucar",
    "macio",
    "macuco",
    "mad max",
    "madagascar",
    "madeira",
    "madeline",
    "madrugada dos mortos",
    "madrugada muito louca",
    "maduro",
    "mae",
    "maestro",
    "magali",
    "maggie",
    "magico",
    "magnetico",
    "magneto",
    "magnolia",
    "mago",
    "magoado",
    "magro",
    "maguila, o gorila",
    "maio",
    "maionese",
    "mais estranho que a ficcao",
    "maisena",
    "majin boo",
    "mala",
    "malabarista",
    "malagueta",
    "malaria",
    "malditas aranhas",
    "malevola",
    "malhar",
    "malhete",
    "malote",
    "maltratar",
    "mamadeira",
    "mamao",
    "mamba",
    "mamma mia",
    "mamute",
    "manada",
    "mancha",
    "manchado",
    "manda chuva",
    "mandando bala",
    "mandark",
    "mandioca",
    "mandioquinha",
    "mandril",
    "mandy",
    "manequim",
    "manete",
    "manga",
    "mangaba",
    "mangostao",
    "mangueira",
    "mangusto",
    "manicure",
    "manivela",
    "manjar",
    "manjericao",
    "manobrista",
    "mansao foster para amigos imaginarios",
    "manta",
    "manteiga",
    "mao",
    "mapa",
    "mapear",
    "maquete",
    "maquiador",
    "maquiagem",
    "maquiar",
    "maquina de costura",
    "maquina de datilografar",
    "maquina de lavar louca",
    "maquina de lavar roupa",
    "maquina fotografica",
    "maquina mortifera",
    "maquinista",
    "mar",
    "mar de fogo",
    "maraca",
    "maracana",
    "maracuja",
    "maratona",
    "marcador de pagina",
    "marcador de texto",
    "marcar",
    "marcelino pao e vinho",
    "marceneiro",
    "marcha atletica",
    "maresia",
    "marfim",
    "margarida",
    "margarina",
    "marge",
    "maria mole",
    "marido por acaso",
    "marimba",
    "marimbondo",
    "marinheiro",
    "marinho",
    "marionete",
    "mariposa",
    "marisco",
    "maritaca",
    "marley e eu",
    "marlin-azul",
    "marmelada",
    "marmelo",
    "marmita",
    "marmota",
    "marreco",
    "marreta",
    "marshmallow",
    "marsupilami",
    "marte",
    "marte ataca!",
    "martelar",
    "martelo",
    "martin mystery",
    "marvin, o marciano",
    "mascara",
    "mascara cirurgica",
    "mascara de mergulho",
    "mascara de oxigenio",
    "mascarado",
    "masha e o urso",
    "massagear",
    "massagista",
    "massoterapeuta",
    "mastigar",
    "mastro",
    "mata",
    "mata atlantica",
    "mata-mata",
    "mata-mosca",
    "mate",
    "material",
    "mato sem cachorro",
    "matrix",
    "max",
    "max payne",
    "max steel",
    "maxilar",
    "maxixe",
    "meany ranheta",
    "mecanico",
    "mecanimais",
    "medabots",
    "medalha",
    "medicar",
    "medico",
    "meditar",
    "medo do escuro",
    "medroso",
    "medusa",
    "megafone",
    "megaman",
    "megamente",
    "megas xlr",
    "meia",
    "meia arrastao",
    "meia-calca",
    "meia-noite em paris",
    "meiga",
    "mel",
    "melancia",
    "melao",
    "mendigo",
    "menina de ouro",
    "meninas malvadas",
    "menino maluquinho - o filme",
    "mensageiro",
    "mensagem para voce",
    "menta",
    "mentiroso",
    "mercurio",
    "merengue",
    "mergulhador",
    "mergulhar",
    "mergulho",
    "mesa",
    "mesa de ping-pong",
    "mesa de sinuca",
    "mesquita",
    "mestre dos magos",
    "mestre kame",
    "mestre karin",
    "mestre splinter ",
    "metalurgico",
    "meteoro",
    "meteorologista",
    "metido",
    "metralhadora",
    "metro",
    "meu amigo da escola e um macaco",
    "meu malvado favorito",
    "meu primeiro amor",
    "meu primeiro homem",
    "meu tio matou um cara",
    "meus 15 anos",
    "mexerica",
    "mexilhao",
    "mib homens de preto",
    "micanga",
    "mickey mouse",
    "mico-leao-dourado",
    "mico-leao-preto",
    "micose",
    "micro-ondas",
    "microbiologista",
    "microfone",
    "microscopio",
    "mike, lu e og",
    "miles do amanha",
    "milho",
    "milhouse",
    "milkshake",
    "mimeografo",
    "mimica",
    "mina",
    "minato",
    "mingau",
    "minha filha quer casar",
    "minha mae e uma peca",
    "minha super ex-namorada",
    "minhas maes e meu pai",
    "minhoca",
    "minions",
    "minissaia",
    "minnie mouse",
    "minotauro",
    "miope",
    "mirai nikki",
    "mirar",
    "mirtilo",
    "miss simpatia",
    "miss spider",
    "missao impossivel",
    "missil",
    "misto quente",
    "misturar",
    "mistureba",
    "moana um mar de aventuras",
    "moby dick",
    "mochila",
    "modelista",
    "modelo",
    "modem",
    "moeda",
    "moer",
    "moinho",
    "mola",
    "moldura",
    "moldureiro",
    "moleiro",
    "molhado",
    "molhar",
    "molheira",
    "molho",
    "molho agridoce",
    "molho barbecue",
    "molho branco",
    "molho de pimenta",
    "molho de tomate",
    "molho ingles",
    "molho madeira",
    "molho rose",
    "molho shoyu",
    "molho tartaro",
    "molinete",
    "molusco",
    "mona a vampiro",
    "monge",
    "monica",
    "monitor",
    "monitorar",
    "monkey d luffy",
    "monoculo",
    "monster high",
    "monster rancher",
    "monstro",
    "monstros s.a.",
    "montador",
    "montanha",
    "montanha-russa",
    "montanhismo",
    "montar",
    "moonlight",
    "moonwalker",
    "moranga",
    "morango",
    "morangos silvestres",
    "moranguinho",
    "morcego",
    "mordaca",
    "mordecai",
    "mordedor",
    "morder",
    "mordida",
    "mordomo",
    "moreia",
    "moreno",
    "morsa",
    "mortadela",
    "mortal kombat",
    "morte negra",
    "mosaico",
    "mosca",
    "mosquiteiro",
    "mosquito",
    "mosquito, mosquete e moscardo",
    "mosquito-prego",
    "mostarda",
    "mostrar",
    "moto",
    "motoboy",
    "motociclismo",
    "motociclista",
    "motocross",
    "motoqueiro fantasma",
    "motor",
    "motorista",
    "moulin rouge",
    "mountain bike",
    "mouse",
    "mouse pad",
    "mousse",
    "mousse de chocolate",
    "mousse de limao",
    "mousse de maracuja",
    "mousse de morango",
    "mr. Magoo",
    "mr. satan",
    "mucarela",
    "mucha lucha",
    "muffin",
    "mula",
    "mula sem cabeca",
    "mulan",
    "muleta",
    "mulher gato",
    "mulher gaviao",
    "mulher invisivel",
    "mulher-gato",
    "multar",
    "multiprocessador",
    "mumia",
    "mumificado",
    "munhequeira",
    "muppet babies",
    "muqueca",
    "mural",
    "muralha",
    "muralha da china",
    "murchar",
    "murcho",
    "murici",
    "muriel",
    "muro",
    "muro das lamentacoes",
    "musaranho",
    "musculo",
    "musculo total",
    "musculoso",
    "museu",
    "museu do louvre",
    "musgo",
    "musicoterapeuta",
    "mussarela",
    "mutano",
    "mutante rex",
    "muttley",
    "my little pony",
    "na natureza selvagem",
    "na trilha da fama",
    "nabo",
    "nachos",
    "nadadeira",
    "nadar",
    "nado sincronizado",
    "naja",
    "nami",
    "namorados",
    "namorar",
    "nanatsu no taizai",
    "narceja",
    "narguile",
    "narina",
    "nariz",
    "nariz de palhaco",
    "naruto",
    "narval",
    "nasce uma estrela",
    "natacao",
    "natal",
    "natsu",
    "naufrago",
    "nautilo",
    "navalha",
    "navegar",
    "navio",
    "navio cargueiro",
    "navio fantasma",
    "neblina",
    "nebulizador",
    "nectarina",
    "ned flanders",
    "negar",
    "nem que a vaca tussa",
    "nermal",
    "nervoso",
    "nespera",
    "neta",
    "neto",
    "netuno",
    "neurocirurgiao",
    "neuronio",
    "nevar",
    "neve",
    "nhoque",
    "nine",
    "ninho",
    "no cair da noite",
    "no pique de nova iorque",
    "no rastro da bala",
    "nobre",
    "nocautear",
    "noe",
    "noite",
    "noite infernal",
    "noiva em fuga",
    "noivas em guerra",
    "norbit",
    "nosferatu",
    "nossa turma",
    "notebook",
    "noturno",
    "novela",
    "novelo de la",
    "novo",
    "noz-moscada",
    "nozes",
    "nublado",
    "nuca",
    "nuggets",
    "nutricionista",
    "nutrir",
    "nuvem",
    "o acampamento de lazlo",
    "o agente teen",
    "o albergue",
    "o amor e cego",
    "o ano em que faremos contato",
    "o aprendiz de feiticeiro",
    "o ataque dos tomates assassinos",
    "o auto da compadecida",
    "o aviador",
    "o balconista",
    "o bebe de rosemary",
    "o beijo da mulher-aranha",
    "o bem amado",
    "o bicho de sete cabecas",
    "o bicho vai pegar",
    "o cacador de pipas",
    "o cacador e a rainha do gelo",
    "o cachorro vira-lata",
    "o caminho para casa",
    "o campeao",
    "o cao e a raposa",
    "o carro desgovernado",
    "o carteiro e o poeta",
    "o castelo animado",
    "o cemiterio maldito",
    "o ceu pode esperar",
    "o chamado",
    "o cheiro do ralo",
    "o cla das adagas voadoras",
    "o clube das winx",
    "o codigo da vinci",
    "o coisa",
    "o colecionador de ossos",
    "o corcunda de notre dame",
    "o corte",
    "o corvo",
    "o curioso caso de benjamin button",
    "o demolidor",
    "o despertar de uma paixao",
    "o dia depois de amanha",
    "o dia que a terra parou",
    "o diabo veste prada",
    "o diario da princesa",
    "o diario de uma baba",
    "o ditador",
    "o dolar furado",
    "o dorminhoco",
    "o equilibrista",
    "o escorpiao rei",
    "o escritor fantasma",
    "o espanta tubaroes",
    "o estranho mundo de jack",
    "o exorcismo de emily rose",
    "o exorcista",
    "o expresso polar",
    "o exterminador do futuro",
    "o fada do dente",
    "o fantasma da opera",
    "o fantastico mundo de bobby",
    "o fazendeiro de deus",
    "o filho de chucky",
    "o garoto",
    "o gato de botas",
    "o grande ditador",
    "o grande dragao branco",
    "o grande polegar",
    "o grinch",
    "o grito",
    "o guarda-costas",
    "o guia do mochileiro das galaxias",
    "o hobbit",
    "o homem bicentenario",
    "o homem da mascara de ferro",
    "o homem elefante",
    "o homem que copiava",
    "o homem que desafiou o diabo",
    "o homem sem sombra",
    "o iluminado",
    "o ilusionista",
    "o incrivel mundo de gumball",
    "o inferno de dante",
    "o invisivel",
    "o jardim secreto",
    "o jardineiro fiel",
    "o justiceiro",
    "o labirinto do fauno",
    "o laboratorio de dexter",
    "o lar das criancas peculiares",
    "o leitor",
    "o leopardo",
    "o livro de eli",
    "o lobisomem",
    "o lobo atras da porta",
    "o lutador",
    "o magico de oz",
    "o mascara",
    "o maskara",
    "o massacre da serra eletrica",
    "o menino da porteira",
    "o menino do pijama listrado",
    "o mentiroso",
    "o misterio das duas irmas",
    "o morro dos ventos uivantes",
    "o mundo de leland",
    "o nevoeiro",
    "o nome da rosa",
    "o orfanato",
    "o pagamento",
    "o pai da noiva",
    "o paizao",
    "o patinho duque",
    "o patriota",
    "o pentelho",
    "o pequenino",
    "o pequeno principe",
    "o pequeno stuart little",
    "o pequeno urso",
    "o pequeno vampiro",
    "o peste",
    "o pestinha",
    "o pianista",
    "o poderoso chefao",
    "o poderoso chefinho",
    "o poderoso hercules",
    "o procurado",
    "o professor aloprado",
    "o quarteto fantastico",
    "o quarto do panico",
    "o quarto poder",
    "o rei leao",
    "o reino proibido",
    "o resgate do soldado ryan",
    "o retorno da mumia",
    "o segredo de brokeback mountain",
    "o senhor dos aneis",
    "o sexto sentido",
    "o show de truman",
    "o silencio dos inocentes",
    "o som do coracao",
    "o sotao",
    "o terminal",
    "o tigre e o dragao",
    "o ultimo cacador de bruxas",
    "o ultimo dos moicanos",
    "o ultimo mestre do ar",
    "o ultimo portal",
    "o ultimo samurai",
    "o ultimo trem",
    "o urso do cabelo duro",
    "o vale dos dinossauros",
    "o vestido",
    "o vidente",
    "o vizinho",
    "oasis",
    "obelisco",
    "obesidade",
    "oboe",
    "observar",
    "obstaculo",
    "oca",
    "ocapi",
    "oceano",
    "oculos",
    "oculos de sol",
    "odiar",
    "odie",
    "odre",
    "office-boy",
    "oftalmologista",
    "ofuro",
    "ogro",
    "olaf",
    "old boy",
    "oleo",
    "oleoso",
    "olha quem esta falando",
    "olhar",
    "olhar de anjo",
    "olheiras",
    "olho",
    "olho de sogra",
    "olho magico",
    "olho vivo e faro fino",
    "olhos famintos",
    "olivia palito",
    "ombreira",
    "ombro",
    "omelete",
    "onca",
    "onda",
    "onde os fracos nao tem vez",
    "onde vivem os monstros",
    "one piece",
    "one punch-man",
    "onibus",
    "onibus espacial",
    "onze homens e um segredo",
    "operacao big hero",
    "operacao cupido",
    "operacao valquiria",
    "operar",
    "orangotango",
    "orar",
    "orca",
    "ordenhar",
    "oregano",
    "orelha",
    "orelhao",
    "orelhudo",
    "orgao",
    "orgulhoso",
    "origami",
    "orix",
    "ornitorrinco",
    "orobo",
    "orochimaru",
    "orquidea",
    "ortopedista",
    "os 101 dalmatas",
    "os 6 bionicos",
    "os anjinhos",
    "os batutinhas",
    "os caca-fantasmas",
    "os caezinhos do canil",
    "os camundongos aventureiros",
    "os castores pirados",
    "os cavaleiros do zodiaco",
    "os condenados",
    "os defensores da terra",
    "os embalos de sabado a noite",
    "os flintstones",
    "os herculoides",
    "os impossiveis",
    "os incriveis",
    "os infiltrados",
    "os jetsons",
    "os jovens titas",
    "os jovens titas em acao",
    "os meninos desordeiros",
    "os mensageiros",
    "os mercenarios",
    "os mussarelas",
    "os normais",
    "os oblongs",
    "os outros",
    "os padrinhos magicos",
    "os passaros",
    "os pinguins de madagascar",
    "os pinguins do papai",
    "os produtores",
    "os reis da rua",
    "os sabados secretos",
    "os seis signos da luz",
    "os sete monstrinhos",
    "os sete samurais",
    "os simpsons",
    "os smurfs",
    "os snorks",
    "os substitutos",
    "os super patos",
    "os thornberrys",
    "os tres mosqueteiros",
    "os vingadores",
    "osso",
    "ostentar",
    "osteoporose",
    "ostra",
    "ostra japonesa",
    "ourico",
    "ourico-do-mar",
    "ourives",
    "ouro",
    "outono",
    "ouvido",
    "ouvir",
    "oval",
    "ovelha",
    "ovo",
    "ovo de codorna",
    "ovo de pascoa",
    "p.s. eu te amo",
    "pa",
    "pa de lixo",
    "paca",
    "pacoca",
    "pacote",
    "pacu",
    "padaria",
    "padeiro",
    "padre",
    "pagar",
    "pai",
    "paintball",
    "pairulito",
    "pais",
    "paisagem",
    "paisagista",
    "paixao de aluguel",
    "palacio",
    "palacio do planalto",
    "palafita",
    "palco",
    "palestrante",
    "paleto",
    "palha",
    "palha italiana",
    "palhaco",
    "palheta",
    "palido",
    "paliteiro",
    "palito de dente",
    "palito de fosforo",
    "palmeira",
    "palmilha",
    "palmito",
    "pamonha",
    "pancreas",
    "panda",
    "panda-vermelho",
    "pandeiro",
    "pandorum",
    "panela",
    "panela de barro",
    "panela de pressao",
    "panetone",
    "panfleto",
    "pangolim",
    "panico",
    "panico a bordo",
    "panico na floresta",
    "panico no lago",
    "pano",
    "pano de chao",
    "pano de prato",
    "panqueca",
    "pantanal",
    "pantano",
    "pantera",
    "pantufa",
    "panturrilha",
    "pao",
    "pao australiano",
    "pao de alho",
    "pao de batata",
    "pao de forma",
    "pao de lo",
    "pao de mel",
    "pao de queijo",
    "pao doce",
    "pao frances",
    "pao integral",
    "pao sirio",
    "pao-de-acucar",
    "papa-capim",
    "papa-leguas",
    "papagaio",
    "papagaio-do-mar",
    "papai noel",
    "papai sabe nada",
    "paparazzi",
    "papel",
    "papel carbono",
    "papel de parede",
    "papel higienico",
    "papel laminado",
    "papel toalha",
    "papelaria",
    "papete",
    "papo de anjo",
    "paquerar",
    "para sempre alice",
    "para-raios",
    "parabenizar",
    "parabrisa",
    "parachoque",
    "parada",
    "parafina",
    "parafuso",
    "paraquedas",
    "paraquedista",
    "parca",
    "parceiro",
    "pardal",
    "parede",
    "parmesao",
    "parque",
    "partir",
    "passa anel",
    "passageiro",
    "passagem",
    "passaporte",
    "passar",
    "passarela",
    "passaro-lira",
    "passear",
    "passeata",
    "pasta",
    "pasta de amendoim",
    "pastel",
    "pastel de belem",
    "pastelaria",
    "pasteleiro",
    "pastilha",
    "pata",
    "pate",
    "patela",
    "pateta",
    "patinete",
    "patins",
    "pato",
    "pato de borracha",
    "pato donald",
    "patolino",
    "patrick",
    "patriota",
    "patrulha canina",
    "paty maionese",
    "pau-brasil",
    "pavao",
    "pave",
    "pe",
    "pe de cabra",
    "pe de galinha",
    "pe de moleque",
    "pe de pano",
    "pe de pato",
    "pe-de-moleque",
    "pe-grande",
    "pedagio",
    "pedagogo",
    "pedal",
    "pedalar",
    "pedalinho",
    "pedestal",
    "pedestre",
    "pediatra",
    "pedicure",
    "pedra",
    "pedra-pomes",
    "pedreiro",
    "pedrita",
    "pega varetas",
    "pegada",
    "pegaso",
    "peito de frango",
    "peito de peru",
    "peixaria",
    "peixe",
    "peixe-agulha",
    "peixe-anjo",
    "peixe-boi",
    "peixe-bolha",
    "peixe-borboleta",
    "peixe-bruxa",
    "peixe-corda",
    "peixe-espada",
    "peixe-folha",
    "peixe-galo",
    "peixe-gato",
    "peixe-lanterna",
    "peixe-leao",
    "peixe-lua",
    "peixe-mandarim",
    "peixe-morcego",
    "peixe-palhaco",
    "peixe-pedra",
    "peixe-rei",
    "peixe-serra",
    "peixe-sol",
    "peixe-trombeta",
    "peixe-voador",
    "peixe-zebra",
    "peixeiro",
    "peixonauta",
    "pele eterno",
    "pelicano",
    "pelo",
    "peludo",
    "pen drive",
    "pena",
    "penalti",
    "pendulo",
    "pendurar",
    "peneira",
    "penelope charmosa",
    "penelope pussycat",
    "penhasco",
    "penhoar",
    "peninsula",
    "pensar",
    "pensativo",
    "pente",
    "penteadeira",
    "penteado",
    "pentear",
    "pepe le gamba",
    "pepe legal",
    "pepino",
    "pepino-do-mar",
    "peppa pig",
    "pepperoni",
    "pequeno",
    "pequenos espioes",
    "pequi",
    "pera",
    "percevejo",
    "percy jackson",
    "perder",
    "perdido pra cachorro",
    "perdiz",
    "perfumado",
    "perfumar",
    "perfumaria",
    "perfume",
    "perfume de mulher",
    "perfumista",
    "perfurador de papel",
    "pergaminho",
    "peridot",
    "periquito",
    "periscopio",
    "perito",
    "perna",
    "perna de pau",
    "pernalonga",
    "pernil",
    "pernilongo",
    "perninha",
    "perola",
    "perola sirigueijo",
    "perry o ornitorrinco",
    "perseguir",
    "persiana",
    "personal trainer",
    "peru",
    "peruca",
    "pesadelo",
    "pesado",
    "pesca",
    "pescador",
    "pescar",
    "pescoco",
    "pesquisar",
    "pessego",
    "peste negra",
    "pet shop",
    "petala",
    "pete puma",
    "peteca",
    "peter griffin",
    "peter pan",
    "peter perfeito",
    "peter potamus",
    "petisqueira",
    "petit gateau",
    "petit suisse",
    "petroleo",
    "pets a vida secreta dos bichos",
    "phineas e ferb",
    "pia",
    "pianista",
    "piano",
    "piao",
    "piau",
    "pica-pau",
    "picanha",
    "picar",
    "picareta",
    "piccolo",
    "picles",
    "picole",
    "piercing",
    "piggley winks",
    "pijama",
    "pilao",
    "pilha",
    "pilotar",
    "piloto",
    "pimenta",
    "pimenta do reino",
    "pimentao",
    "pinca",
    "pincel",
    "pincelar",
    "pingente",
    "pingu",
    "pinguim",
    "pinha",
    "pinhao",
    "pinheiro",
    "pinky dinky doo",
    "pinky e o cerebro",
    "pino",
    "pinoquio",
    "pintado",
    "pintar",
    "pintor",
    "piolho",
    "piolho-de-cobra",
    "piorar",
    "pipa",
    "pipeta",
    "pipira",
    "pipoca",
    "pipoca doce",
    "pipoqueira",
    "pipoqueiro",
    "piramide",
    "piranha",
    "pirao",
    "pirarucu",
    "pirata",
    "piratas do caribe",
    "pires",
    "pirulito",
    "pisar",
    "pisca-pisca",
    "piscar",
    "piscina",
    "pista",
    "pistache",
    "pistola",
    "pitanga",
    "pitaya",
    "pitomba",
    "piton",
    "piu-piu",
    "pixarro",
    "pixcodelics",
    "pixels",
    "pizza",
    "pizzaiolo",
    "pizzaria",
    "placa",
    "plagiar",
    "planador",
    "planet survival",
    "planeta",
    "planeta dos macacos",
    "planeta sheen",
    "planeta terror",
    "plankton",
    "plano de voo",
    "planta",
    "planta carnivora",
    "plantacao",
    "plantar",
    "plataforma",
    "plateia",
    "plic, ploc e chuvisco",
    "plucky",
    "pluma",
    "plumagem",
    "plutao",
    "pluto",
    "pneu",
    "pobre",
    "poca",
    "pocahontas",
    "pocao",
    "pochete",
    "poco",
    "pocoyo",
    "podadeira",
    "podar",
    "poder alem da vida",
    "podio",
    "poeira",
    "pokemon",
    "polaina",
    "pole position",
    "polegar",
    "polen",
    "polenta",
    "policial",
    "policiar",
    "polir",
    "politico",
    "polly pocket",
    "polo aquatico",
    "poltrona",
    "polvilho",
    "polvo",
    "pomar",
    "pombo",
    "pompom",
    "poncheira",
    "ponei",
    "ponte",
    "ponteiro",
    "pontudo",
    "popeye",
    "por um fio",
    "porao",
    "poraque",
    "porco",
    "porco-da-terra",
    "porco-do-mar",
    "porco-do-mato",
    "porco-espinho",
    "porcos e diamantes",
    "porquinho-da-india",
    "porta",
    "porta cd",
    "porta giratoria",
    "porta-avioes",
    "porta-lapis",
    "porta-luvas",
    "porta-malas",
    "porta-retrato",
    "portao",
    "porteira",
    "porteiro",
    "porto",
    "postar",
    "poste",
    "poster",
    "posto de gasolina",
    "pote",
    "potro",
    "pousada",
    "pousar",
    "praca",
    "praca de alimentacao",
    "praia",
    "prancha",
    "prancheta",
    "prata",
    "prateleira",
    "prato",
    "preciosa",
    "predador",
    "predio",
    "prefeito",
    "pregador",
    "pregar",
    "prego",
    "preguicoso",
    "premonicao",
    "prenda-me se for capaz",
    "prendedor de cabelo",
    "prender",
    "presente",
    "presentear",
    "presepio",
    "presidente",
    "presidio",
    "presilha",
    "pressagio",
    "presto",
    "presuntinho",
    "presunto",
    "primavera",
    "princesa",
    "princesa caroco",
    "princesa jujuba",
    "princesas-do-mar",
    "princesinha sofia",
    "principe",
    "prisma",
    "procurador",
    "procurando dory",
    "procurando nemo",
    "professor",
    "professor dingle dong",
    "professor utonio",
    "profiteroles",
    "programa",
    "proibido",
    "proibir",
    "projetil",
    "projeto zeta",
    "prologo dos ceus",
    "prometheus",
    "pronunciar",
    "proteger",
    "protegido",
    "protese",
    "protestar",
    "protetor auricular",
    "protetor bucal",
    "protetor solar",
    "prova de fogo",
    "provolone",
    "psicologo",
    "psicose",
    "psiquiatra",
    "publicitario",
    "pucca",
    "pudim",
    "pudim de pao",
    "puff",
    "pular",
    "pular corda",
    "pulga",
    "pulmao",
    "pulp fiction",
    "pulseira",
    "pulso",
    "pulverizador",
    "puma",
    "punhal",
    "pupila",
    "pure",
    "puro osso",
    "puxador",
    "puxar",
    "quadril",
    "quadrilha",
    "quadrilha de morte",
    "quadro",
    "quadrupede",
    "quando me apaixono",
    "quartel",
    "quarto",
    "quati",
    "quatro bracos",
    "quebra-cabeca",
    "quebra-nozes",
    "quebrado",
    "quebrando a banca",
    "quebrar",
    "queijadinha",
    "queijeira",
    "queijo",
    "queijo ralado",
    "queimada",
    "queimado",
    "queimar",
    "queime depois de ler",
    "queixada",
    "queixo",
    "quem quer ser um milionario?",
    "quem vai ficar com mary?",
    "quentao",
    "quente",
    "quepe",
    "quero-quero",
    "quetzal",
    "quiabo",
    "quibe",
    "quicar",
    "quieto",
    "quimono",
    "quindim",
    "quinoa",
    "quintal",
    "quiosque",
    "quitanda",
    "quivi",
    "ra",
    "rabada",
    "rabanada",
    "rabanete",
    "rabeca",
    "rabiscar",
    "rabo",
    "racao",
    "rachadura",
    "rachar",
    "rack",
    "radar",
    "radio",
    "radio amador",
    "radio gravador",
    "raia",
    "rainha",
    "raio",
    "raio x",
    "raiz",
    "ralador",
    "ralar",
    "ralo",
    "ralo de pia",
    "ramalhete",
    "rambo",
    "rampa",
    "ranger",
    "ranma",
    "rapadura",
    "rapel",
    "rapido",
    "raposa",
    "raquete",
    "raquete eletrica",
    "rasgado",
    "rasgar",
    "raso",
    "raspar",
    "rasteirinha",
    "rastejar",
    "rastelo",
    "rasurar",
    "ratatouille",
    "ratazana",
    "ratel",
    "rato",
    "rato minuto",
    "rato-toupeira-pelado",
    "ratoeira",
    "ravena",
    "ravioli",
    "ray",
    "rebanho",
    "rebobinar",
    "rebocar",
    "rebolar",
    "reboque",
    "recem-nascido",
    "recepcionista",
    "rechear",
    "reciclar",
    "reciclavel",
    "reclamar",
    "reco-reco",
    "recortar",
    "recruta zero",
    "recusar",
    "redakai",
    "rede",
    "rede de descanso",
    "rede de pesca",
    "redea",
    "redondo",
    "reduzir",
    "refletir",
    "reflexo",
    "refratario",
    "refrigerante",
    "regador",
    "regar",
    "registrar",
    "regua",
    "rei",
    "rejuvenescer",
    "relaxar",
    "relicario",
    "religioso",
    "relogio",
    "relogio de pulso",
    "relogio de sol",
    "relojoaria",
    "relojoeiro",
    "remedio",
    "remo",
    "remover",
    "rena",
    "repolho",
    "repolho roxo",
    "reporter",
    "reprovar",
    "requeijao",
    "resgatar",
    "resgate abaixo de zero",
    "resident evil",
    "respirar",
    "ressecar",
    "restaurante",
    "reto",
    "retornar",
    "retrato",
    "retroprojetor",
    "retrovisor",
    "revista",
    "revisteiro",
    "revolver",
    "rezar",
    "riacho",
    "rick e morty",
    "rico",
    "ricota",
    "rifle",
    "rigby",
    "rim",
    "rimel",
    "ringue",
    "rinoceronte",
    "rio",
    "rios vermelhos",
    "riquinho",
    "riquinho rico",
    "risoles",
    "risonho",
    "risoto",
    "robalo",
    "robin",
    "robin hood",
    "robo",
    "robocop",
    "robot boy",
    "rocambole",
    "rock lee",
    "rock star",
    "rocket power",
    "rocky",
    "roda-gigante",
    "rodape",
    "rodar",
    "rodeio",
    "rodo",
    "rodoviaria",
    "roer",
    "rolar",
    "roleta",
    "rolha",
    "rolinho primavera",
    "rolo compressor",
    "rolo de massa",
    "roma",
    "romantico",
    "rondele",
    "rosa",
    "rosario",
    "rosbife",
    "rosquinha",
    "roteador",
    "rotular",
    "rotulo",
    "rouba bandeira",
    "roupao",
    "roy corroi",
    "rua",
    "rubeola",
    "rucula",
    "ruga",
    "ruiva",
    "rum",
    "rupert",
    "ryu",
    "s.o.s. do amor",
    "sabao",
    "saboneteira",
    "saborear",
    "saboroso",
    "sabre",
    "saca-rolhas",
    "sacada",
    "saci perere",
    "saciar",
    "saco de plastico",
    "sacro",
    "sacudir",
    "saga de gemeos",
    "sagitario",
    "sagrado",
    "sagu",
    "sagui",
    "sagwa",
    "saia",
    "saicanga",
    "saida de emergencia",
    "saiga",
    "sailor moon",
    "sair",
    "saitama",
    "sakura card captors",
    "sakura kinomoto",
    "sal",
    "sal de frutas",
    "sal grosso",
    "sala",
    "sala de aula",
    "salada",
    "salada de frutas",
    "saladeira",
    "saladeiro",
    "salamandra",
    "salame",
    "salario",
    "saleiro",
    "salgadeiro",
    "salgadinho",
    "salgar",
    "salmao",
    "salpicao",
    "salsa",
    "salsao",
    "salsicha",
    "saltar",
    "salto alto",
    "salto com vara",
    "salto em altura",
    "salto triplo",
    "salva-vidas",
    "salvar",
    "samambaia",
    "samba cancao",
    "sambar",
    "samurai",
    "samurai champloo",
    "samurai jack",
    "samurai x",
    "sandalia",
    "sanduiche",
    "sanduicheira",
    "sandy bochechas",
    "sanfona",
    "sanfoneiro",
    "sangue",
    "sangue negro",
    "sanguessuga",
    "sanjay e craig",
    "sanji",
    "santo",
    "santo matrimonio",
    "saori",
    "sapatear",
    "sapateira",
    "sapateiro",
    "sapatilha",
    "sapato",
    "sapo",
    "sapo-boi",
    "sapoti",
    "saque",
    "saracura",
    "sarampo",
    "sarapatel",
    "sardinha",
    "sargento",
    "sarna",
    "sashimi",
    "sasuke",
    "satelite",
    "satiro",
    "saturno",
    "saudavel",
    "sauna",
    "saxofone",
    "scanner",
    "scooby-doo",
    "se beber, nao case!",
    "se ela danca eu danco",
    "se eu fosse voce",
    "se liga ian",
    "secador de cabelo",
    "secadora",
    "secar",
    "seco",
    "secretaria",
    "sede de sangue",
    "seguir",
    "seguranca",
    "seguranca de shopping",
    "segurar",
    "seiya",
    "selecionar",
    "selo",
    "sem novidade no front",
    "sem saida",
    "semaforo",
    "semear",
    "semente",
    "sempre ao seu lado",
    "senhores do crime",
    "sentado",
    "sentar",
    "separados pelo casamento",
    "separar",
    "sepultar",
    "sequilho",
    "sereia",
    "serenata",
    "seriema",
    "seringa",
    "seringueira",
    "seringueiro",
    "serpente",
    "serpentes a bordo",
    "serpentina",
    "serra",
    "serra eletrica",
    "serrar",
    "serrote",
    "serval",
    "servir",
    "sete vidas",
    "sexta-feira 13",
    "sexta-feira muito louca",
    "shakespeare apaixonado",
    "shaman king",
    "shazam",
    "she-ra",
    "sheep na cidade grande",
    "shenlong",
    "sherlock holmes",
    "shikamaru",
    "shin chan",
    "shino",
    "shinzo",
    "shopping",
    "short",
    "show da luna",
    "shrek",
    "shura de capricornio",
    "shurato",
    "silencio dos inocentes",
    "silverhawks",
    "simba",
    "sin city",
    "sinais",
    "sininho",
    "sino",
    "sinusite",
    "sirene",
    "siri",
    "siriguela",
    "skate",
    "smoking",
    "snoopy",
    "snowboard",
    "sobrancelha",
    "sobrar",
    "sobre meninos e lobos",
    "sobremesa",
    "sobretudo",
    "socar",
    "sociedade dos poetas mortos",
    "soco ingles",
    "socorrer",
    "socorrista",
    "sofa",
    "sogra",
    "soja",
    "sokka",
    "sol",
    "soldado",
    "soldado universal",
    "soldador",
    "soldar",
    "solido",
    "solitario",
    "sombra",
    "sombrero",
    "sombrinha",
    "sonambulo",
    "sonhar",
    "sonho",
    "sonic",
    "sopa",
    "sopa de legumes",
    "soro",
    "sorridente",
    "sorrir",
    "sorriso",
    "sorte no amor",
    "sortudo",
    "sorvete",
    "sorvete napolitano",
    "sorveteira",
    "sorveteiro",
    "sorveteria",
    "south park",
    "sozinho",
    "space ghost",
    "space jam",
    "spawn",
    "speed racer",
    "spiff e hercules",
    "spray",
    "squash",
    "sr sirigueijo",
    "sr. Burns",
    "sr. e sra. smith",
    "sr. sinistro",
    "stan smith",
    "star trek",
    "star vs as forcas do mal",
    "star wars",
    "stardust",
    "stargate",
    "steven universe",
    "stewie",
    "stigmata",
    "strogonoff",
    "suado",
    "subir",
    "submarino",
    "suco",
    "suco de abacaxi",
    "suco de acerola",
    "suco de goiaba",
    "suco de graviola",
    "suco de laranja",
    "suco de limao",
    "suco de maca",
    "suco de maracuja",
    "suco de morango",
    "suco de pitanga",
    "suco de uva",
    "sucuri",
    "sueter",
    "sufle",
    "sujar",
    "sujo",
    "sukiyaki western django",
    "sumir",
    "sumo",
    "sundae",
    "sunga",
    "suor",
    "super campeoes",
    "super choque",
    "super dinamo",
    "super esquadrao dos macacos robos",
    "super homem",
    "super onze",
    "super pig",
    "super sumo",
    "super vaca",
    "superamigos",
    "supermercado",
    "supernoobs",
    "suplicar",
    "suportar",
    "surf",
    "surfar",
    "surfista prateado",
    "suricato",
    "surucucu",
    "sushi",
    "sushiman",
    "suspensorio",
    "suspirar",
    "suspiro",
    "sutia",
    "swat kats",
    "sweeney todd",
    "ta chovendo hamburguer",
    "ta dando onda",
    "ta todo mundo louco",
    "tablet",
    "tabua",
    "tabuleiro",
    "taca",
    "taca de sangue",
    "tacaca",
    "taco",
    "tainha",
    "talco",
    "talharim",
    "talher",
    "tamanco",
    "tamandua",
    "tamanduai",
    "tamara",
    "tamarindo",
    "tambaqui",
    "tambor",
    "tamborim",
    "tampa",
    "tampar",
    "tanajura",
    "tanga",
    "tangara",
    "tangerina",
    "tanque",
    "tapeceiro",
    "tapete",
    "tapete vermelho",
    "tapioca",
    "tarantula",
    "tartaruga",
    "tartaruga touche",
    "tartarugas ninja",
    "tarzan",
    "tatame",
    "tatu",
    "tatuador",
    "tatuagem",
    "tatuar",
    "taturana",
    "taxi",
    "taximetro",
    "taxista",
    "taz",
    "tear",
    "teatro",
    "tecido",
    "teclado",
    "teclar",
    "tecnico",
    "ted",
    "teia",
    "teimoso",
    "teiu",
    "tela",
    "teleferico",
    "telefonar",
    "telefone",
    "telefonista",
    "telejornal",
    "telescopio",
    "televisao",
    "telha",
    "telhado",
    "temaki",
    "temari",
    "temos vagas",
    "temperar",
    "tempero",
    "tempestade",
    "templo",
    "tempo",
    "tempos modernos",
    "tenchi muyo",
    "tenda",
    "tenente",
    "tenis",
    "tenis de mesa",
    "tenista",
    "tentaculos",
    "teologo",
    "tequila",
    "terapeuta",
    "terk",
    "terminar",
    "termometro",
    "terno",
    "terra",
    "terraco",
    "terremoto",
    "terror sobre rodas",
    "tesoura",
    "tesoureiro",
    "tesouro",
    "testa",
    "teto",
    "texugo",
    "the notebook",
    "the tick",
    "the wonders",
    "thomas e seus amigos",
    "thor",
    "thundercats",
    "tiara",
    "tibia",
    "tico e teco",
    "tico-tico",
    "tigela",
    "tigrao",
    "tigre",
    "tigre branco",
    "tigre de bengala",
    "tijolo",
    "tilapia",
    "timao e pumba",
    "timido",
    "timmy turner",
    "tingido",
    "tinker bell",
    "tinta",
    "tinta de cabelo",
    "tintureiro",
    "tiny toon",
    "tio chan",
    "tio patinhas",
    "tipografo",
    "tipoia",
    "tiramisu",
    "tirolesa",
    "tiros em ruanda",
    "titanic",
    "titio avo",
    "toalha",
    "toalha de mesa",
    "tobi",
    "toboga",
    "tocar",
    "tocha",
    "tocha humana",
    "todo mundo em panico",
    "todo poderoso",
    "todos os caes merecem o ceu",
    "tofu",
    "toga",
    "toldo",
    "tom e jerry",
    "tomada",
    "tomate",
    "tomate cereja",
    "tomate seco",
    "tomates verdes fritos",
    "tomilho",
    "top gun",
    "topografo",
    "toranja",
    "torax",
    "torcer",
    "torcida",
    "tornado",
    "torneira",
    "tornozeleira",
    "tornozelo",
    "torpedo",
    "torrada",
    "torradeira",
    "torrar",
    "torre",
    "torre de pisa",
    "torre eiffel",
    "torresmo",
    "torrone",
    "torta",
    "torta alema",
    "torta de limao",
    "torta de maracuja",
    "torta de pessego",
    "tortilha",
    "tortilla",
    "torto",
    "tosador",
    "tosse",
    "tossir",
    "totoro",
    "touca",
    "toupeira",
    "toureiro",
    "touro",
    "touro indomavel",
    "toy story",
    "trabalhador",
    "trabalhar",
    "traca",
    "trailer",
    "trair",
    "traira",
    "trama internacional",
    "tramar",
    "trampolim",
    "tranca",
    "trancado",
    "trancar",
    "transatlantico",
    "transferidor",
    "transformers",
    "transito",
    "transmitir",
    "transparente",
    "transpirar",
    "transportar",
    "trapezio",
    "tratamento de choque",
    "trator",
    "trave",
    "travesseiro",
    "treinando o papai",
    "treinar",
    "trem",
    "trem-bala",
    "tremer",
    "treno",
    "tres espias demais",
    "trevo",
    "triangulo",
    "triatlo",
    "tribo",
    "tribunal",
    "triceps",
    "triciclo",
    "tricotar",
    "tridente",
    "trigo",
    "trigun",
    "trilho",
    "trio eletrico",
    "tripe",
    "triste",
    "tritao",
    "triturado",
    "triturador",
    "triturar",
    "trocar",
    "trofeu",
    "troia",
    "trolls",
    "tromba",
    "trombone",
    "trompa",
    "trompete",
    "tron",
    "tronco",
    "trono",
    "tropa de elite",
    "tropecar",
    "trufa",
    "trunks",
    "truta",
    "tsuna",
    "tsunade senju",
    "tsunami",
    "tuba",
    "tubarao",
    "tubarao-baleia",
    "tubarao-branco",
    "tubarao-duende",
    "tubarao-enfermeiro",
    "tubarao-lixa",
    "tubarao-martelo",
    "tubo de ensaio",
    "tucano",
    "tucunare",
    "tucupi",
    "tuiuiu",
    "tulipa",
    "tumba",
    "tunel",
    "tunica",
    "turbante",
    "turbina",
    "turbo",
    "turista",
    "turistas",
    "turma da monica",
    "tutubarao",
    "twf",
    "uca",
    "uisque",
    "uivar",
    "ulquiorra",
    "ultimo",
    "ultrapassar",
    "ultravioleta",
    "um amor para recordar",
    "um corpo que cai",
    "um crime de mestre",
    "um dia de furia",
    "um doce olhar",
    "um estranho no ninho",
    "um faz de conta que acontece",
    "um homem serio",
    "um hotel bom pra cachorro",
    "um kickboxer muito louco",
    "um lobisomem americano em paris",
    "um olhar do paraiso",
    "um sonho de liberdade",
    "um sonho possivel",
    "um tira da pesada",
    "um tira no jardim de infancia",
    "uma aventura lego",
    "uma baba quase perfeita",
    "uma carta ao pai",
    "uma chamada perdida",
    "uma linda mulher",
    "uma mente brilhante",
    "uma noite alucinante",
    "uma noite magica",
    "uma noite no museu",
    "uma robo adolescente",
    "uma vida iluminada",
    "umbigo",
    "umbu",
    "unha",
    "unha postica",
    "unicornio",
    "uniforme",
    "uniformizado",
    "universidade",
    "universidade monstros",
    "universo",
    "up altas aventuras",
    "urano",
    "urna",
    "ursinho pooh",
    "ursinhos carinhosos",
    "ursinhos gummy",
    "urso",
    "urso-pardo",
    "urso-polar",
    "urubu",
    "urubu-rei",
    "urucum",
    "urumutum",
    "urutu",
    "usina hidreletrica",
    "uva",
    "uva passa",
    "v de vinganca",
    "vaca",
    "vaca atolada",
    "vacinar",
    "vaga-lume",
    "vagao",
    "vagem",
    "vai que cola",
    "valentino troca-tapa",
    "vampira",
    "vampiro",
    "vaqueiro",
    "varal",
    "varanda",
    "varrer",
    "vash",
    "vasilha",
    "vaso",
    "vaso sanitario",
    "vassoura",
    "vatapa",
    "vaticano",
    "vazio",
    "vegeta",
    "vegetariano",
    "veia de lutador",
    "vela",
    "veleiro",
    "velejador",
    "velho",
    "velma",
    "velocidade maxima",
    "velocipede",
    "velocista",
    "velorio",
    "velotrol",
    "veloz",
    "velozes e furiosos",
    "vem dancar",
    "vem dancar comigo",
    "vencedor",
    "vencer",
    "vendedor",
    "vender",
    "veneno",
    "venenoso",
    "venom",
    "ventar",
    "ventilador",
    "ventilador de teto",
    "ventilar",
    "vento",
    "ventriloquo",
    "venus",
    "verao",
    "vereador",
    "vesgo",
    "vespa",
    "vespa-do-mar",
    "vestiario",
    "vestibular",
    "vestida para casar",
    "vestido",
    "vestir",
    "veterinario",
    "veu",
    "viaduto",
    "viagem ao centro da terra",
    "viagem maldita",
    "viajar",
    "viatura",
    "vicunha",
    "vida de inseto",
    "video cassete",
    "video game",
    "vidraceiro",
    "vidro",
    "viela",
    "vigia",
    "vigiar",
    "viking",
    "vila",
    "vilao",
    "vinagre",
    "vinagrete",
    "vingador",
    "vinho",
    "vinho branco",
    "vinicius e tom",
    "viola",
    "violao",
    "violino",
    "violoncelo",
    "viseira",
    "visitar",
    "vitamina",
    "vitral",
    "vitrola",
    "viuva-negra",
    "voando alto",
    "voar",
    "vodca",
    "volante",
    "volcano",
    "volei de praia",
    "voleibol",
    "voltar",
    "volume",
    "vombate",
    "vomitar",
    "voo noturno",
    "votar",
    "vovo uranai",
    "vulcao",
    "w.i.t.c.h.",
    "waffles",
    "wall-e",
    "wallaby",
    "wally gator",
    "wanda",
    "wasabi",
    "watchmen",
    "webcam",
    "wifi ralph",
    "wilma",
    "windsurf",
    "wolverine: imortal",
    "woodstock",
    "woody",
    "x-men",
    "xadrez",
    "xale",
    "xarope",
    "xavier",
    "xerife",
    "xerocar",
    "xexeu",
    "xicara",
    "xilofone",
    "xuxa abracadabra",
    "xuxa e os duendes",
    "yakisoba",
    "yeti",
    "yin yang yo!",
    "yu yu hakusho",
    "yu-gi-oh!",
    "yumi",
    "yzma",
    "zabumba",
    "zangao",
    "zarabatana",
    "zatch bell",
    "ze carioca",
    "ze colmeia",
    "ze jacare",
    "zebra",
    "zebu",
    "zeca urubu",
    "zelador",
    "zero no tsukaima",
    "ziguezaguear",
    "ziper",
    "zohan o agente bom de corte",
    "zoologico",
    "zootopia",
    "zorrilho",
    "zuko",
    "zumbi",
    "zumbilandia",
    "zurg",
  ];

let suggestionBox = null;
let isExtensionActive = false;

// Função para criar a caixa de sugestões
function createSuggestionBox() {
    if (suggestionBox) {
        suggestionBox.remove();
    }

    suggestionBox = document.createElement('div');
    suggestionBox.id = 'word-game-assistant';
    suggestionBox.innerHTML = `
        <div class="wga-header">
            <div class="wga-title">
                <span class="wga-icon">🎯</span>
                Word Assistant
            </div>
            <button class="wga-toggle" id="wga-toggle">
                <span class="toggle-icon">⚡</span>
            </button>
        </div>
        <div class="wga-content" id="wga-content">
            <div class="wga-info">
                <div class="wga-pattern" id="wga-pattern">Aguardando dica...</div>
                <div class="wga-count" id="wga-count">0 sugestões</div>
            </div>
            <div class="wga-suggestions" id="wga-suggestions">
                <div class="wga-no-hints">Nenhuma dica detectada ainda</div>
            </div>
        </div>
    `;

    document.body.appendChild(suggestionBox);

    // Toggle da extensão
    document.getElementById('wga-toggle').addEventListener('click', () => {
        isExtensionActive = !isExtensionActive;
        const content = document.getElementById('wga-content');
        const toggleBtn = document.getElementById('wga-toggle');
        
        if (isExtensionActive) {
            content.style.display = 'block';
            suggestionBox.classList.add('active');
            toggleBtn.innerHTML = '<span class="toggle-icon">⚡</span>';
            startMonitoring();
        } else {
            content.style.display = 'none';
            suggestionBox.classList.remove('active');
            toggleBtn.innerHTML = '<span class="toggle-icon">💤</span>';
            stopMonitoring();
        }
    });
}

// Função para normalizar texto (remover acentos, etc.)
function normalizeText(text) {
    return text.toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9\s]/g, '');
}

// Função para extrair padrão da dica
function extractPattern() {
    const dicaElement = document.querySelector('.dicaAtual');
    if (!dicaElement) return null;

    // Extrair número de letras
    const h2 = dicaElement.querySelector('h2');
    if (!h2) return null;

    const match = h2.textContent.match(/\((\d+)\s+letras?\)/);
    if (!match) return null;

    const totalLetters = parseInt(match[1]);

    // Extrair letras já preenchidas
    const spans = dicaElement.querySelectorAll('.traco');
    if (!spans || spans.length === 0) return null;

    let pattern = '';
    let filledPositions = [];

    spans.forEach((span, index) => {
        const text = span.textContent.trim();
        if (text && text !== '' && text !== '_' && text !== '*') {
            pattern += text.toLowerCase();
            filledPositions.push({ index, letter: text.toLowerCase() });
        } else {
            pattern += '_';
        }
    });

    return {
        totalLetters,
        pattern,
        filledPositions,
        patternLength: spans.length
    };
}

// Função para encontrar sugestões
function findSuggestions(patternData) {
    if (!patternData) return [];

    const { totalLetters, pattern, filledPositions } = patternData;
    const suggestions = [];

    words.forEach(word => {
        const normalizedWord = normalizeText(word);
        
        // Verificar se o comprimento bate
        if (normalizedWord.length !== totalLetters) return;

        // Verificar se as letras preenchidas batem
        let matches = true;
        for (let pos of filledPositions) {
            if (pos.index < normalizedWord.length) {
                const wordChar = normalizedWord[pos.index];
                if (wordChar !== pos.letter && pos.letter !== ' ') {
                    matches = false;
                    break;
                }
            }
        }

        if (matches) {
            suggestions.push({
                original: word,
                normalized: normalizedWord,
                confidence: calculateConfidence(normalizedWord, pattern)
            });
        }
    });

    // Ordenar por confiança
    return suggestions.sort((a, b) => b.confidence - a.confidence);
}

// Função para calcular confiança da sugestão
function calculateConfidence(word, pattern) {
    let matches = 0;
    let total = 0;

    for (let i = 0; i < Math.min(word.length, pattern.length); i++) {
        if (pattern[i] !== '_') {
            total++;
            if (word[i] === pattern[i]) {
                matches++;
            }
        }
    }

    return total > 0 ? (matches / total) * 100 : 0;
}

// Função para atualizar a interface
function updateSuggestions() {
    if (!isExtensionActive) return;

    const patternData = extractPattern();
    const patternElement = document.getElementById('wga-pattern');
    const countElement = document.getElementById('wga-count');
    const suggestionsElement = document.getElementById('wga-suggestions');

    if (!patternData) {
        patternElement.textContent = 'Aguardando dica...';
        countElement.textContent = '0 sugestões';
        suggestionsElement.innerHTML = '<div class="wga-no-hints">Nenhuma dica detectada ainda</div>';
        return;
    }

    // Mostrar padrão atual
    const visualPattern = patternData.pattern.replace(/_/g, '•').toUpperCase();
    patternElement.textContent = `Padrão: ${visualPattern} (${patternData.totalLetters} letras)`;

    // Encontrar sugestões
    const suggestions = findSuggestions(patternData);
    countElement.textContent = `${suggestions.length} sugestão${suggestions.length !== 1 ? 'ões' : ''}`;

    // Mostrar sugestões
    if (suggestions.length === 0) {
        suggestionsElement.innerHTML = '<div class="wga-no-hints">Nenhuma palavra encontrada para este padrão</div>';
    } else {
        const suggestionsHtml = suggestions.slice(0, 10).map((suggestion, index) => {
            const confidenceClass = suggestion.confidence >= 80 ? 'high' : 
                                  suggestion.confidence >= 50 ? 'medium' : 'low';
            
            return `
                <div class="wga-suggestion ${confidenceClass}" data-word="${suggestion.original}">
                    <div class="suggestion-text">${suggestion.original}</div>
                    <div class="suggestion-confidence">${Math.round(suggestion.confidence)}%</div>
                </div>
            `;
        }).join('');

        suggestionsElement.innerHTML = suggestionsHtml;

        // Adicionar evento de clique para copiar
        suggestionsElement.querySelectorAll('.wga-suggestion').forEach(el => {
            el.addEventListener('click', () => {
                const word = el.dataset.word;
                navigator.clipboard.writeText(word).then(() => {
                    el.classList.add('copied');
                    setTimeout(() => el.classList.remove('copied'), 1000);
                });
            });
        });
    }
}

// Observer para mudanças na página
let observer = null;

function startMonitoring() {
    if (observer) return;

    observer = new MutationObserver((mutations) => {
        let shouldUpdate = false;

        mutations.forEach((mutation) => {
            if (mutation.type === 'childList' || mutation.type === 'characterData') {
                // Verificar se houve mudança na área de dicas
                const target = mutation.target;
                if (target.closest && target.closest('.dicaAtual')) {
                    shouldUpdate = true;
                }
            }
        });

        if (shouldUpdate) {
            setTimeout(updateSuggestions, 100);
        }
    });

    observer.observe(document.body, {
        childList: true,
        subtree: true,
        characterData: true
    });

    // Atualização inicial
    updateSuggestions();
}

function stopMonitoring() {
    if (observer) {
        observer.disconnect();
        observer = null;
    }
}

// Inicializar quando a página carregar
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            createSuggestionBox();
        }, 1000);
    });
} else {
    setTimeout(() => {
        createSuggestionBox();
    }, 1000);
}

// Atalho de teclado para ativar/desativar (Ctrl+Shift+W)
document.addEventListener('keydown', (e) => {
    if (e.ctrlKey && e.shiftKey && e.key === 'W') {
        e.preventDefault();
        const toggleBtn = document.getElementById('wga-toggle');
        if (toggleBtn) {
            toggleBtn.click();
        }
    }
});